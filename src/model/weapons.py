...
WEAPON_DEFS: dict[str, WeaponDef] = {
    ...
    "Science_TC_Control": WeaponDef(
        name="Control Shot", weapon_type="two_click", damage=0, range_max=2,
        upgrade_a="+1 move",
        upgrade_b="+1 move",
    ),
...
def get_weapon_def(weapon_id: str) -> WeaponDef | None:
    ...
    The Phase 3 runtime overlay (``set_runtime_overrides`` /
    ``_RUNTIME_OVERRIDES``) used to live here so the Python sim could
    mirror Rust-side overrides. With the Python sim deleted, only the
    Rust solver consults overrides — they reach it via bridge JSON,
