"""Seal the counterbalanced complete enemy-tournament runtime campaign."""
...
from src.observatory.enemy_tournament_hw import (
    ANALYSIS_KIND,
...
class EnemyTournamentHwCampaignError(RuntimeError):
    """Raised when committed tournament evidence is missing or inconsistent."""


def _stable_bytes(path: Path, label: str) -> bytes:
    candidate = Path(os.path.abspath(path))
    if candidate.is_symlink() or not candidate.is_file():
        raise EnemyTournamentHwCampaignError(
    ...
    if (
        ...
        raise EnemyTournamentHwCampaignError(f"{label} changed while read")
    return data
...
def _load(path: Path, label: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise EnemyTournamentHwCampaignError(f"invalid {label}: {exc}") from exc
    if not isinstance(value, dict):
        raise EnemyTournamentHwCampaignError(f"{label} must be an object")
    return value
...
def _exact_children(root: Path, expected: set[str], *, directories: bool) -> None:
    if root.is_symlink() or not root.is_dir():
        raise EnemyTournamentHwCampaignError(f"campaign path is invalid: {root}")
    matching = {
        item.name
...
def _created_at(trial: Mapping[str, Any], label: str) -> datetime:
    ...
    if type(value) is not str:
        raise EnemyTournamentHwCampaignError(f"{label} created_at is invalid")
    try:
...
def _metadata_digest(
    ...
    if value.get("sha256") != digest:
        raise EnemyTournamentHwCampaignError(f"{label} digest differs")
    return value
...
def _validate_trial(
    ...
    *,
    ...
    suffix = pair_name[-3:]
    capture_id = f"tournament-pair-{suffix}-{condition}"
    expected_counts = (
        {"candidate_count": 8, "selected_count": 1, "queue_count": 1}
        if condition == "armed"
        else {"candidate_count": 0, "selected_count": 0, "queue_count": 0}
    )
    expected_ack = (
        "OK OBS_ENEMY_TOURNAMENT_TRIAL "
    ...
    if (
        ...
        or trial.get("kind") != "observatory_enemy_tournament_turn_trial"
        or trial.get("pair_id") != f"tournament-pair-{suffix}"
...
def build_enemy_tournament_hw_campaign_receipt(
    campaign_root: Path,
    *,
    ...
    build = _load(build_path, "enemy-tournament build receipt")
    plan = _load(plan_path, "enemy-tournament breakpoint plan")
    if (
        _sha256(_stable_bytes(build_path, "enemy-tournament build receipt"))
        != EXPECTED_BUILD_RECEIPT_SHA256
        or build.get("module_sha256") != EXPECTED_MODULE_SHA256
        or build.get("hardware_breakpoint_plan_sha256")
        != EXPECTED_PLAN_SHA256
        or _sha256(_stable_bytes(plan_path, "enemy-tournament breakpoint plan"))
        != EXPECTED_PLAN_SHA256
        or plan.get("kind")
        != "observatory_enemy_tournament_hardware_breakpoint_plan"
    ):
        raise EnemyTournamentHwCampaignError(
            "enemy-tournament build identity differs"
        )
    ...
    for pair_name, expected_order in PAIR_SPECS.items():
        ...
        if analysis != committed_analysis or analysis.get("kind") != ANALYSIS_KIND:
            raise EnemyTournamentHwCampaignError(
                f"{pair_name} enemy-tournament analysis drift"
            )
        ...
        comparisons = {
            "control_dormant": compare_rng_trial_outcomes(
                outcomes["control"],
                outcomes["dormant"],
                capture_id=f"enemy-tournament-{pair_name}-dormant",
            ),
            "control_armed": compare_rng_trial_outcomes(
                outcomes["control"],
                outcomes["armed"],
                capture_id=f"enemy-tournament-{pair_name}-armed",
            ),
        }
        ...
        pairs.append(
            {
                "pair": pair_name,
                "pair_id": f"tournament-pair-{pair_name[-3:]}",
    ...
    return {
        ...
        "claims": {
            ...
            "not_proven": [
                ...
                "Concrete intermediate Lua PointLists, SkillEffects, callback values, or candidate-time Board observations beyond the final records captured in this scenario.",
        ...
        "solver_conformance": {
            "resolved_rule": (
                ...
                "selected record to bind to the settled queue"
            ),
            "offline_model": (
...
def publish_enemy_tournament_hw_campaign_receipt(
