"""Exact large backward SIMD copy replay with a separate snapshot and ordered-access oracle."""
...
ANALYSIS_KIND = "pe_native_large_backward_simd_copy_conformance"
SEALED_SHA256 = "d14a3ebfe55f89f4d60f06686343af07258c94d750f462150476671fd228df91"
START = 0x36E580
RANGES = ((3597696, 3597728), (3598388, 3598417), (3598565, 3598795))
...
def vectors():
    return [
        dict(
            length=n,
            source_offset=0xF00 + a,
            destination_offset=0xF00 + a + delta,
            alignment=frame,
            df=0,
            feature_word=word,
        )
...
def oracle(vector, payload=PAYLOAD_TEMPLATE):
    ...
    _require(type(n) is int and 128 <= n <= 2048, "invalid short SIMD length")
    _require(
        type(source) is int
    ...
    _require(
        source < destination < source + n,
        "outside strict backward overlap interval",
    )
    _require(
    ...
    while position >= 128:
        position -= 128
        for part in range(8):
            accesses.append(("read", position + 16 * part, 16))
        for part in range(8):
            accesses.append(("write", position + 16 * part, 16))
        xmm_values.update(
            {
                f"xmm{part}": int.from_bytes(
                    snapshot[position + 16 * part : position + 16 * (part + 1)],
                    "little",
                )
                for part in range(8)
            }
    ...
    while position >= 32:
        ...
        xmm_values.update(
            xmm0=int.from_bytes(snapshot[position : position + 16], "little"),
            xmm1=int.from_bytes(snapshot[position + 16 : position + 32], "little"),
        )
    while position >= 4:
        position -= 4
    ...
    return dict(
        ...
        flags=0x44,
        flag_mask=0x8C5 if remaining % 4 == 0 else 0x8D5,
    )
...
def _run_case(codes, points, vector, *, negative=None):
    ...
    def on_code(m, address, size, user):
        ...
        if (
            negative == "byte"
            and pc == 0x36E910
            and m.reg_read(x.UC_X86_REG_ECX)
            == vector["length"] - ((destination + vector["length"]) & 15)
        ):
            at = m.reg_read(x.UC_X86_REG_ESI)
            m.mem_write(at, bytes([m.mem_read(at, 1)[0] ^ 1]))
        if negative == "xmm" and pc == 0x36E9CA:
    ...
    wanted_xmm = dict(initial_xmm, **expected["xmm_values"])
...
def _build_unsealed(executable, semantics):
    from src.observatory import native_large_backward_simd_copy_semantics as sem
    ...
    result = dict(
        ...
        scope=dict(
            ...
            claim="Exact large backward MOVDQU path matches independent snapshot, XMM and ordered access oracles",
            premises=[
                "Explicit lengths 128 through 2048 with strict higher-destination overlapping intervals",
            ...
            xmm_policy="All eight XMM registers checked; each lowest 128 byte block replaces all eight XMM registers, later 32 byte blocks replace only XMM0 and XMM1; every other register retains its prior value",
            not_claimed=[
                "Forward copy, greater lengths, other feature paths or overread permissions",
