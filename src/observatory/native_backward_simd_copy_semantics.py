"""Backward overlapping short MOVDQU copy paths with an independent snapshot oracle."""
...
SCHEMA_VERSION = 1
ANALYSIS_KIND = "pe_native_backward_simd_copy_semantics"
SEALED_SHA256 = "5e5fc334a1d99e6d920826c1f96b1c97693e9b041fc58c9453a9fc6d0e2849bd"
SOURCE_PINS = {
...
class BackwardSimdCopyError(RuntimeError):
    pass


def _require(ok, message):
    if not ok:
        raise BackwardSimdCopyError(message)


def _normalize(fn):
    try:
        return fn()
    except BackwardSimdCopyError:
        raise
    except Exception as exc:
        raise BackwardSimdCopyError(str(exc)) from exc
...
def model_case(
    ...
    while not returned:
        ...
        if op == "push":
            ...
            raise BackwardSimdCopyError("unsupported scalar operation")
        pc = next_pc
    stack = fixture["stack"]
    ...
    for row in rows:
        ...
        for arg in row.operands:
            if arg.type == x86.X86_OP_REG:
                ...
                raise BackwardSimdCopyError("unexpected instruction operand")
        pc = row.address - BASE
        _require(
...
def _build_unsealed(executable, sources):
    ...
    vectors += [
        ...
        for n in (32, 33, 35, 36, 47, 48, 63, 64, 65, 95, 96, 97, 127)
        for a in range(16)
        for delta in (1, 3, 17)
        for frame in (0, 15)
    ]
    ...
    _require(union == [f"0x{pc:08x}" for pc in ORDER], "backward SIMD coverage differs")
    controls = []
    for name, pc, replacement in [
        ("wrong_vector_source", 0x36E977, ("movdqu", R("xmm0"), M("esi", 1, width=16))),
        ("wrong_vector_store", 0x36E984, ("movdqu", M("edi", 16, width=16), R("xmm0"))),
        (
        ...
        try:
            model_case(96, 256, 257, ops=changed)
        except BackwardSimdCopyError:
            controls.append(dict(name=name, rejected=True))
        else:
            raise BackwardSimdCopyError("backward SIMD mutation accepted")
    result = {
        ...
        "scope": {
            "evidence_class": "bounded_backward_MOVDQU_copy_graph_with_snapshot_oracle",
            "premises": [
                "Length is 32 through 127 and source is below destination below source exclusive end; both exclusive ends are representable unsigned words",
                ...
                "All byte alignments allowed; mapped readable source and writable destination intervals disjoint from protected stable frame, code and feature word",
                "Finite frame base is 0x30001000 with listed alignments",
            ],
            "relation": "Backward suffix bytes align destination exclusive end to 16, then paired vector loads precede paired stores for descending 32 byte blocks, followed by descending scalar remainder; destination equals original source snapshot with all other payload unchanged",
            "xmm_relation": "With no full 32 byte block all XMM registers preserve; otherwise XMM0 and XMM1 contain the final original lowest 32 byte block halves and XMM2 through XMM7 preserve",
            "registers": "EAX returns destination, ECX is zero, EDX remains original length and other GPRs restore with cdecl ESP advancement of four",
            "flags": "CF OF SF are zero, PF ZF one; AF undefined when remaining scalar byte count is divisible by four, zero otherwise; DF remains zero",
            "not_claimed": [
                "Forward copy, 128 byte vector blocks, other feature paths or overread permissions",
