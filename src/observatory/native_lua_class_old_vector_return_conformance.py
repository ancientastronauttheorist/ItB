"""Finite native class transfer, small old-vector growth, and normal caller return."""
...
ANALYSIS_KIND = "pe_native_lua_class_old_vector_return_conformance"
SEALED_SHA256 = "ebdd6a4cfe4ce14ce5606b183ddea87fd4babafa13adf12e6e6980050c62945a"
SOURCE_PINS = {
    "program_facts": prefix.SOURCE_PINS["program_facts"],
    "empty_vector_return": (empty.ANALYSIS_KIND, empty.SEALED_SHA256),
    "growth_conformance": (growth.ANALYSIS_KIND, growth.SEALED_SHA256),
    "deallocation_conformance": growth.SOURCE_PINS["deallocation_conformance"],
    "deallocation_composition": growth.SOURCE_PINS["deallocation_composition"],
}
...
def vectors():
    return [
        dict(
            ...
            old_size=size,
            old_alignment=(7 + 13 * alignment) % 32,
        )
...
def _fixture(vector):
    ...
    old_begin = growth.OLD + 0x100 + old_alignment
    for offset, value in (
        (4, old_begin),
        (8, old_begin + 8 * size),
        (12, old_begin + 8 * size),
...
def _load_code(data, image, sources):
    ...
    bodies = [
        sources["empty_vector_return"]["body"],
    ...
    for body in bodies:
        for point in body["points"]:
            _require(
                ...
                "old vector witness conflict",
            )
            witnesses[point["rva"]] = point
    ranges = [
        (int(r["start_rva"], 16), int(r["end_rva"], 16))
        for r in sources["empty_vector_return"]["body"]["ranges"]
    ]
    ranges += list(growth.resize.deallocator.BODIES.values())
...
def _build_unsealed(executable, sources):
    ...
    result = dict(
        ...
        scope=dict(
            claim="Native class transfer, full nonnull small-vector growth with successful deallocation, and normal caller return on a finite canonical corpus",
            premises=[
                ...
                "Vector initially contains zero through three old records with size equal to capacity; one fresh block retains those records and appends the external argument",
            ...
            not_claimed=[
                "Assertion, internal argument, more than three old records, allocator metadata, failed allocation or deallocation, exceptions, arbitrary strings, aliased trees, or hardware execution",
