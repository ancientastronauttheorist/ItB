"""Integrated successful resize for four through 256 live elements with feature-zero scalar copying."""
...
from src.observatory import native_scalar_copy_semantics as copy_contract
from src.observatory import native_scalar_copy_conformance as copy_replay
...
ANALYSIS_KIND = "pe_native_scalar_vector_resize_conformance"
SEALED_SHA256 = "2916af83679014cc7ba06402933ed3194e1d7240b0bcd121317926d93dc81a28"
SOURCE_PINS = {
    ...
    "scalar_copy": (copy_contract.ANALYSIS_KIND, copy_contract.SEALED_SHA256),
...
def geometry(vector):
    ...
    _require(
        4 <= size <= 256 and size <= cap <= 512 and size <= n <= 513,
        "outside scalar resize geometry",
    )
    _require(
...
def _expected(
    ...
    def copy_chunk(offset, width):
        at = source - OLD + offset
        word = int.from_bytes(old[at : at + width], "little")
        event("read", source + offset, word, width)
        event("write", destination + offset, word, width)
        return word
    ...
    for i in range(words):
        last_word = copy_chunk(h + 4 * i, 4)
    if remaining >= 32:
        table = 0x36E7E4
        r(BASE + table + 4 * tail, BASE + copy_replay.TABLES[table][tail])
    for i in range(tail):
        copy_chunk(h + 4 * words + i, 1)
    copy_edx = tail if remaining >= 32 else last_word
    r(s - 32, g["new_begin"])
    r(s - 44, obj)
    r(s - 40, g["new_begin"])
...
def _run_case(codes, points, vector, negative=None):
    ...
    table_payloads = {
        ...
        for t, targets in copy_replay.TABLES.items()
    }
...
def _build_unsealed(executable, sources):
    ...
    for table, targets in copy_replay.TABLES.items():
    ...
    controls = []
    sample = next(v for v in vectors() if v["old_size"] == 4 and v["requested"] == 512)
    for kind, message in (
    ...
    result = dict(
        ...
        scope=dict(
            claim="Native resize and allocation and deallocation chains execute with feature-zero scalar copy; only successful heap API responses remain supplied",
            domain="Sampled live sizes four through 256 elements; old capacity equals size or 512, requested capacity equals size or 512 or 513; all block alignments paired over three stack alignments",
            frame_relation="Owner S reaches allocation API S-76, copy saved state S-44 and free API S-68, returning S+8",
            oracle_join="Sealed allocation and deallocation oracles are rebased with real continuations; independent alignment prefix, ascending DWORD, dispatch-table read and byte-tail equations join full memory states",
            premises=[
                "Disjoint old and new buffers with destination below source; valid large-storage metadata; complete ancestor stack and object",
                "Stable zero copy feature words, immutable exact dispatch tables, heap and import slots; DF clear",
                "Supplied successful stdcall API responses preserve memory and nonvolatile registers",
            ],
            not_claimed=[
                "Actual heap effects, failed API paths, SIMD feature modes, larger payloads or arbitrary geometry",
                "Complete vector resize equivalence outside sampled cases or whole-program accounting promotion",
            ],
        ),
    )
    _require(
