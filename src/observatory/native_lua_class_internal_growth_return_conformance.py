"""Finite native class transfer, internal argument reallocation, and normal caller return."""
...
from src.observatory import (
    native_lua_class_internal_spare_return_conformance as internal,
)
...
ANALYSIS_KIND = "pe_native_lua_class_internal_growth_return_conformance"
SEALED_SHA256 = "28a3febc6f81de40c0a2ac69270ec3c0a4c71efe5a9f67393915f6ae3f719330"
SOURCE_PINS = {
    "program_facts": prefix.SOURCE_PINS["program_facts"],
    "old_vector_return": (old.ANALYSIS_KIND, old.SEALED_SHA256),
    "internal_spare_return": (internal.ANALYSIS_KIND, internal.SEALED_SHA256),
}
...
def vectors():
    return [
        dict(
            v,
            vector_alignment=(0, 31)[(i + size + index) % 2],
            old_size=size,
...
def _fixture(vector):
    ...
    _require(
        ...
        and 0 <= index < size,
        "invalid internal growth vector geometry",
    )
    fixture = prefix._fixture(vector)
    pages = {p: bytearray(v) for p, v in fixture["pages"].items()}
    old_begin = ARGUMENT - 8 * index
    _require(
        ARGUMENT - 16 <= old_begin and old_begin + 8 * size <= ARGUMENT + 24,
        "internal old vector escaped reserved records",
    )
    for offset, value in (
        (4, old_begin),
        (8, old_begin + 8 * size),
    ...
    for i, b in enumerate(construction.IMPORT.to_bytes(4, "little")):
        a = growth.FREE_IAT + i
        pages[a & ~0xFFF][a & 0xFFF] = b
    ...
    fixture.update(
        ...
        vector_begin=fresh,
        vector_end=fresh + 8 * size,
        vector_capacity=fresh + 8 * (size + 1),
        old_begin=old_begin,
        old_size=size,
        argument_index=index,
    )
...
def _expected(vector, fixture):
    ...
    _require(ARGUMENT < old_end, "argument is not internal")
    ...
    _require(
        index == vector["argument_index"] and begin == fixture["old_begin"],
        "internal growth record index differs",
    )
    _require(
        old_end == read(receiver + 12) == begin + 8 * vector["old_size"],
        "internal old vector is not full",
    )
    # ECX holds the old begin, and EDI becomes the record index before reallocation.
    write(frame - 36, begin)
    write(frame - 40, BASE + 0x2EB1DF)
    regs.update(eax=old_end, esi=receiver, ecx=receiver + 4, edi=index, esp=frame - 40)
    child_vector = dict(
        has_old=True,
        old_size=vector["old_size"],
        old_capacity=vector["old_size"],
        requested=vector["old_size"] + 1,
    ...
    child["events"][-1] = dict(
        access="read", address=frame - 40, width=4, value=BASE + 0x2EB1DF
    )
    ...
    _require(
        regs["esp"] == frame - 32 and regs["edi"] == index,
    ...
    new_begin = read(receiver + 4)
    _require(new_begin == fixture["vector_begin"], "internal copy did not rebase begin")
    first = read(new_begin + index * 8)
    write(end, first)
    second = read(new_begin + index * 8 + 4)
    write(end + 4, second)
    write(receiver + 8, read(receiver + 8) + 8)
    regs.update(eax=second, ecx=new_begin, edx=end, esi=receiver, edi=index)
...
def _load_code(data, image, sources):
    ...
    bodies = [
        sources["old_vector_return"]["body"],
        sources["internal_spare_return"]["body"],
    ]
    witnesses = {}
    for body in bodies:
        for point in body["points"]:
            _require(
                ...
                "internal growth witness conflict",
            )
            witnesses[point["rva"]] = point
    for ins in owner:
        if 0x2EB1D6 <= ins.address - BASE < 0x2EB1DF:
            witnesses[_point(ins)["rva"]] = _point(ins)
    ranges = [
        (int(r["start_rva"], 16), int(r["end_rva"], 16))
        for r in sources["old_vector_return"]["body"]["ranges"]
        if not START <= int(r["start_rva"], 16) < 0x2EB22D
    ]
    ranges += [(START, 0x2EB1F7), (0x2EB216, 0x2EB22D)]
    ...
    selected = [
        _point(r)
        for r in owner
        if prefix.STOP <= r.address - BASE < 0x2EB1F7
        or 0x2EB216 <= r.address - BASE < 0x2EB22D
    ]
...
def _build_unsealed(executable, sources):
    ...
    # Full old vectors use scalar zero-to-24-byte copies and exactly one successful native deallocation join; no feature or table reads occur.
    ...
    _require(required <= set(union), "class internal growth normal coverage differs")
    _require(
        not any(
            0x2EB15F <= int(pc, 16) < 0x2EB179 or 0x2EB1F7 <= int(pc, 16) < 0x2EB216
            for pc in union
        ),
    ...
    result = dict(
        ...
        body=dict(
            ...
            points=points,
            internal_growth_return_points=selected,
        ),
        vectors=vectors(),
        ...
        scope=dict(
            claim="Native class transfer, full small-vector growth with internal argument rebased after successful deallocation, and normal caller return on a finite canonical corpus",
            premises=[
                ...
                "Vector initially contains one through three old records around the argument in the source-object page; every record index is represented and the appended record is read from the new allocation",
            ...
            not_claimed=[
                "Assertion, external argument, more than three old records, allocator metadata, failed allocation or deallocation, exceptions, arbitrary strings, aliased trees, or hardware execution",
