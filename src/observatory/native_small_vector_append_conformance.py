...
import capstone
from src.observatory import native_small_vector_growth_conformance as linked_growth
...
ANALYSIS_KIND = "pe_native_small_vector_append_conformance"
SEALED_SHA256 = "d1c606a60b79315b5605cb48bf7506a7454a248153af3a30c5fc211fde4277ed"
SOURCE_PINS = {
...
def _build_unsealed(executable, sources):
    ...
    result = dict(
        ...
        scope=dict(
            claim="The append slice executes native growth, resize, allocation, short copy and deallocation; only successful heap API responses are supplied",
            domain="Zero through three live elements; full small vectors grow, capacity-four vectors append directly; aligned internal first or last elements or independent external argument pages",
            ...
            premises=[
                "Disjoint mapped object, old and new buffers, external argument pages and full ancestor stack; DF clear",
...
def _run_case(codes, points, vector, negative=None):
    ...
    def on_code(m, address, size, user):
        ...
        if address == IMPORT:
            ...
            return
        pc = address - BASE
        _require(pc in allowed, "execution escaped integrated successful append")
    ...
    for _ in range(3):
        ...
        start = resume
    _require(
        machine.reg_read(x.UC_X86_REG_EIP) == STOP, "integrated append did not return"
    )
    _require(
