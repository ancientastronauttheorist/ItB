...
def test_cleanup_receipt_closes_coordinate_campaigns_and_owner_state():
    receipt = _load(CLEANUP_RECEIPT)
    assert receipt["kind"] == "observatory_spawn_coordinate_rng_cleanup_receipt"
    ...
    assert set(campaigns) == {"spawn_coordinate", "spawn_coordinate_rng"}
    pending = {
    ...
    restored_path = _artifact(
        receipt["install_restore"]["post_cleanup_inventory"]
    )
