...
import sys
import pytest
from src.observatory import native_scalar_vector_resize_conformance as helper
...
def test_large_metadata_and_payload_extents_fit_mapped_blocks(alignment):
    ...
    assert g["copy_bytes"] == 2048 and g["new_end"] - g["new_begin"] == 2048
    assert g["new_capacity"] <= g["new_raw"] + g["request"]


@pytest.mark.parametrize("alignment", range(4))
def test_small_destination_exercises_byte_prefix_and_tail(alignment):
    g = helper.geometry(
        vector(old_size=4, old_capacity=4, requested=4, new_alignment=alignment)
    )
    h = (-g["new_begin"]) & 3
    assert h == (4 - alignment) % 4
    assert g["copy_bytes"] == 32
    assert (g["copy_bytes"] - h < 32) == bool(alignment)
    assert g["new_begin"] < g["old_begin"]
...
def receipts():
    suffix = {
        ...
        "conformance": "native_scalar_vector_resize_conformance",
    }
...
def test_integrated_receipt_only_summarizes_heap_apis(receipts):
    ...
    assert evidence["summary"]["executed_sites"] == 248
    assert evidence["summary"]["allocation_api_summaries"] == 8640
    assert evidence["summary"]["free_api_summaries"] == 8640
...
def test_exact_cli_rebuild(receipts):
    ...
    args = [
        sys.executable,
        str(ROOT / "scripts/itb_native_scalar_vector_resize_conformance.py"),
        "build",
        "--executable",
        executable,
    ]
