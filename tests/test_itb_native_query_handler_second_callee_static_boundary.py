"""Exact and adversarial tests for the query-handler second callee."""
from __future__ import annotations
import copy, hashlib, json, os, threading
from pathlib import Path
import pytest
from scripts import itb_native_query_handler_second_callee_static_boundary as cli
from src.observatory import native_query_handler_second_callee_static_boundary as callee
from src.observatory.native_query_handler_second_callee_static_boundary import NativeQueryHandlerSecondCalleeStaticBoundaryError
...
EXE_SHA = "31fe352655982398fb3ee8b0bbe80efd5d65e3a9aa11e3dc39d0364354493fe9"; RAW = "39daf451a37440201d5cadedf946da30d3fa90e1a23677bf39f913f4a8fa6d33"; CANONICAL = "fd8836f3ccaa14ec45931d611f96122b7b64f2ca54331d6aa2730197c1f45b20"
def _read(path): return json.loads(path.read_text(encoding="utf-8"))
@pytest.fixture(scope="module")
def values():
    paths = {"inventory": INVENTORIES / (PREFIX + "full_decompile_baseline_20260830.json"), "facts": PROGRAMS / (PREFIX + "program_facts.json"), "direct": PROGRAMS / (PREFIX + "native_lua_direct_call_census.json"), "query_handler": PROGRAMS / (PREFIX + "native_query_new_handler_static_boundary.json"), "evidence": PROGRAMS / (PREFIX + "native_query_handler_second_callee_static_boundary.json")}
...
def _structure(v, evidence=None): return callee.validate_native_query_handler_second_callee_static_boundary_structure(v["evidence"] if evidence is None else evidence, *_common(v))
...
def _reject(monkeypatch, v, evidence):
    with pytest.raises(NativeQueryHandlerSecondCalleeStaticBoundaryError): _fast(monkeypatch, v, evidence)
...
def test_receipts_full_owner_partition_import_and_nonclaims(values):
    ...
    assert hashlib.sha256(raw).hexdigest() == RAW; assert raw == callee.encode_native_query_handler_second_callee_static_boundary(evidence).encode(); assert callee._canonical_sha256(evidence) == CANONICAL; assert _structure(values)["evidence_sha256"] == CANONICAL
    body, graph = evidence["function_bodies"][0], evidence["control_flow_graphs"][0]
    assert (body["entry_rva"], body["body_size"], body["body_sha256"], body["control_flow_graph_canonical_sha256"]) == ("0x00388bc5", 23, callee._BODY, callee._CFG); assert (graph["node_count"], graph["edge_count"], len(body["reviewed_points"])) == (9, 8, 9)
    ...
    scan = evidence["whole_atlas_reference_scan"]; assert scan["aggregates"] == {"reference_count": 26, "direct_call_count": 26, "comparison_count": 0, "other_address_count": 0, "memory_operand_count": 0, "owner_count": 26}; assert len(scan["references"]) == len(scan["owner_partition"]) == 26; assert len({row["owner_entry_rva"] for row in scan["owner_partition"]}) == 26
    predecessor = evidence["query_handler_second_callee_predecessor_edge"]; assert (predecessor["source_entry_rva"], predecessor["instruction"]["rva"], predecessor["target_entry_rva"]) == ("0x0038bc08", "0x0038bc1a", "0x00388bc5")
    records = evidence["native_calls"]["absolute_pointer_or_memory_syntax"]; assert records[0]["operand_rva"] == "0x004b70a8" and records[0]["file_backed"] is False; assert records[1]["pe_import_binding"] == {"evidence_class": "fact", "library": "KERNEL32.dll", "name": "EnterCriticalSection", "hint": 238, "ordinal": None, "iat_rva": "0x003d6084"}
...
def test_exact_rebuild(values):
    ...
    rebuilt = callee.build_native_query_handler_second_callee_static_boundary(EXE, *_common(values), inventory=values["inventory"]); assert rebuilt == values["evidence"]; assert callee.validate_native_query_handler_second_callee_static_boundary(EXE, values["evidence"], *_common(values), inventory=values["inventory"])["status"] == "verified"
...
@pytest.mark.parametrize("path", [("schema_version",), ("function_bodies", 0, "body_size"), ("function_bodies", 0, "reviewed_points", 0, "sha256"), ("query_handler_second_callee_predecessor_edge", "instruction", "sha256"), ("whole_atlas_reference_scan", "owner_partition", 0, "owner_entry_rva")])
...
def test_structure_requires_direct_prerequisite_status(values, monkeypatch):
    ...
    with pytest.raises(NativeQueryHandlerSecondCalleeStaticBoundaryError, match="prerequisite"): _structure(values)
@pytest.mark.parametrize("path", [("schema_version",), ("analysis_kind",), ("build_identity",), ("atlas",), ("direct_call_census",), ("query_handler_static_boundary",), ("query_handler_second_callee_predecessor_edge",), ("decoder",), ("function_bodies",), ("control_flow_graphs",), ("native_calls",), ("whole_atlas_reference_scan",), ("method",), ("summary",)])
...
def test_encoder_normalizes_inherited_json_tree_error(monkeypatch):
    ...
    with pytest.raises(NativeQueryHandlerSecondCalleeStaticBoundaryError, match="bad JSON tree"): callee.encode_native_query_handler_second_callee_static_boundary({})
...
def test_cli_inherited_errors_and_existing_preservation(tmp_path, monkeypatch, values):
    _root(monkeypatch, tmp_path); output = tmp_path / "e.json"; rendered = callee.encode_native_query_handler_second_callee_static_boundary(values["evidence"]); cli._write_immutably(output, rendered, values["evidence"]); output.write_bytes(rendered.encode() + b" ")
    with pytest.raises(NativeQueryHandlerSecondCalleeStaticBoundaryError, match="refusing to overwrite"): cli._write_immutably(output, rendered, values["evidence"])
    ...
    with pytest.raises(NativeQueryHandlerSecondCalleeStaticBoundaryError, match="inherited root failure"): cli._write_immutably(tmp_path / "other.json", rendered, values["evidence"])
...
def test_cli_assertion_helper_error_is_normalized_and_preserves_existing(tmp_path, monkeypatch, values):
    _root(monkeypatch, tmp_path); output = tmp_path / "e.json"; rendered = callee.encode_native_query_handler_second_callee_static_boundary(values["evidence"]); output.write_bytes(rendered.encode())
    ...
    with pytest.raises(NativeQueryHandlerSecondCalleeStaticBoundaryError, match="inherited regular failure"): cli._write_immutably(output, rendered, values["evidence"])
    assert output.read_bytes() == rendered.encode()
def test_cli_lock_contention(tmp_path, monkeypatch, values):
    _root(monkeypatch, tmp_path); output = tmp_path / "e.json"; rendered = callee.encode_native_query_handler_second_callee_static_boundary(values["evidence"]); output.write_bytes(rendered.encode()); seen = []; original = cli._read_locked_json_document
    def writer():
        if os.name == "nt":
            try:
...
def test_cli_final_corruption_preserves_existing_or_cleans_private_publication(tmp_path, monkeypatch, values, existing):
    _root(monkeypatch, tmp_path); output = tmp_path / "e.json"; rendered = callee.encode_native_query_handler_second_callee_static_boundary(values["evidence"])
    ...
    with pytest.raises(NativeQueryHandlerSecondCalleeStaticBoundaryError, match="final content validation"): cli._write_immutably(output, rendered, values["evidence"])
