...
def _snapshot(*, rng_before: int = 2) -> dict:
    ...
    return {
        ...
        "observer_version": "observatory-enemy-tournament-hw-observer/1",
        "capture_id": "tournament-test-001",
        "identity": {
            "platform": "windows",
            "architecture": "x86",
            "build_id": "13725832",
...
def test_snapshot_replays_complete_ordered_tournament_and_rng_state_exactly():
    result = validate_enemy_tournament_snapshot(
...
def test_snapshot_rejects_replay_queue_order_and_integrity_drift(mutator, message):
    ...
    with pytest.raises(EnemyTournamentHwError, match=message):
        validate_enemy_tournament_snapshot(
...
def test_snapshot_rejects_extra_fields_and_receipt_safety_drift():
    ...
    with pytest.raises(EnemyTournamentHwError, match="snapshot fields differ"):
        validate_enemy_tournament_snapshot(
            snapshot, build_receipt=_receipt(), observed_module_sha256=MODULE_SHA
        )
    ...
    with pytest.raises(EnemyTournamentHwError, match="safety attestation"):
        validate_enemy_tournament_snapshot(
...
def test_correlator_binds_exact_native_replay_to_bridge_queue():
    result = correlate_enemy_tournament_snapshot(
        _snapshot(),
        _outcome(),
        build_receipt=_receipt(),
        observed_module_sha256=MODULE_SHA,
