...
def test_refresh_bridge_capped_repair_disable_clears_status_repair():
    session = RunSession()
    ...
    bridge_data = {
        ...
        "units": [
            {"uid": 1, "hp": 1, "max_hp": 5, "mech": True, "fire": True},
        ],
    }

    result = commands._refresh_bridge_capped_repair_disable(
        session, bridge_data, turn=3
    )

    assert result["removed_count"] == 1
    assert result["current_reasons"] == []
    assert session.disabled_actions == []


def test_refresh_bridge_capped_repair_disable_replaces_stale_reason():
    session = RunSession()
    session.disabled_actions = [
        {
            "weapon_id": "_REPAIR",
    ...
    assert result["removed_count"] == 1
    assert result["current_reasons"] == ["uid=2:hp=2:bridge_max=2:save_max=4"]
    assert session.disabled_actions == [
        {
            "weapon_id": "_REPAIR",
            "cause_pattern": "bridge_repair_cap:uid=2:hp=2:bridge_max=2:save_max=4",
            "expires_turn": 102,
            "strategic_override": True,
        }
    ]
