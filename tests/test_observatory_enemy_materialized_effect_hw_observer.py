...
from scripts import build_itb_observatory_enemy_materialized_effect_hw_observer as observer
...
BUILDER = ROOT / "scripts" / "build_itb_observatory_enemy_materialized_effect_hw_observer.py"
...
def _identities() -> dict:
    return {
        ...
        "skill_effect_canonical_sha256": "4" * 64,
        "skill_effect_file_sha256": "5" * 64,
        "selected_queue_source_sha256": "1" * 64,
        "materialized_prebytes": observer.EXPECTED_MATERIALIZED_PREBYTES,
...
def test_hot_path_captures_ordered_vector_rng_states_and_four_seams_api_free():
    text = SOURCE.read_text(encoding="ascii")
    hot = text[
        text.index("/* MATERIALIZED_EFFECT_HOT_PATH_BEGIN") : text.index(
            "/* MATERIALIZED_EFFECT_HOT_PATH_END */"
        )
    ]

    assert '#pragma code_seg(push, ".obshot")' in hot
    assert "context->Dr0 = (DWORD)g_materialized_effect_selector_address" in hot
    assert "context->Dr1 = (DWORD)g_selected_address" in hot
    assert "context->Dr2 = (DWORD)g_queue_address" in hot
    assert "context->Dr3 = (DWORD)g_materialized_effect_address" in hot
    assert "context->Dr7 = MATERIALIZED_EFFECT_DR7_ARM" in hot
    assert "context->Dr7 = MATERIALIZED_EFFECT_DR7_AFTER_SELECTOR" in hot
    assert "context->Dr7 = MATERIALIZED_EFFECT_DR7_WAIT_MATERIALIZED" in hot
    assert "context->Dr7 = MATERIALIZED_EFFECT_DR7_QUEUE_ONLY" in hot
    assert "g_materialized_effect_rng_before" in hot
    assert "g_materialized_effect_rng_after" in hot
    assert "g_materialized_effect_candidates[index]" in hot
    assert "record->committed = index + 1" in hot
    assert "g_materialized_effect_stage = MATERIALIZED_EFFECT_STAGE_COMPLETE" in hot
    for forbidden in (
        "HeapAlloc",
        "VirtualQuery",
        "CreateFile",
...
def test_capture_layout_matches_reviewed_selector_vector_and_rng_owner_contract():
    ...
    assert "owner_result + MATERIALIZED_EFFECT_RNG_STATE_OFFSET" in text
...
def test_plan_pins_selector_selected_queue_and_owner_state_without_mutation():
    ...
    assert plan["kind"] == "observatory_enemy_materialized_effect_hardware_breakpoint_plan"
    ...
    assert plan["debug_register_contract"]["wait_materialized_dr7_exact"] == (
        "0x00000050"
    )
...
def test_generated_include_pins_all_code_and_evidence_dependencies():
    ...
    assert "#define OBS_MATERIALIZED_EFFECT_SELECTOR_RVA 0x000f7dd0u" in include
    assert "#define OBS_SELECTED_RVA 0x000f6854u" in include
    assert "#define OBS_QUEUE_RVA 0x00227d20u" in include
    assert "#define OBS_MATERIALIZED_RVA 0x00268323u" in include
    ...
    assert '#define OBS_SKILL_EFFECT_BOUNDARY_SHA256 "' + "4" * 64 + '"' in include
    assert '#define OBS_SELECTED_QUEUE_SOURCE_SHA256 "' + "1" * 64 + '"' in include
    assert "OBS_MATERIALIZED_EFFECT_SELECTOR_PREBYTES[6]" in include
    assert "OBS_MATERIALIZED_PREBYTES[10]" in include
...
def test_builder_rejects_an_unpinned_executable(tmp_path):
    ...
    skill_effect = tmp_path / "skill_effect.json"
...
def test_receipt_and_module_names_are_content_addressed(monkeypatch, tmp_path):
    module = b"pinned-materialized_effect-observer"
    ...
    args = argparse.Namespace(
        ...
        skill_effect_boundary=tmp_path / "skill_effect.json",
    ...
    assert receipt["skill_effect_boundary_canonical_sha256"] == "4" * 64
    assert receipt["skill_effect_boundary_file_sha256"] == "5" * 64
