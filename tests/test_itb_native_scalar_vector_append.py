...
import sys
import pytest
from src.observatory import native_scalar_vector_append_conformance as helper
...
def vector(size=16, capacity=16, kind="internal", index=15):
    return dict(
        has_old=True,
        old_size=size,
...
def test_internal_argument_tracks_element_across_relocation(size, grow):
    v = vector(size, size if grow else 512, index=size - 1)
    g = helper.geometry(v)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"old_size": 257, "old_capacity": 257, "requested": 385},
    ],
)
...
def test_matrix_covers_internal_and_both_external_classifications():
    ...
    assert len(values) == 11520
    assert {v["argument_kind"] for v in values} == {
        "internal",
        "external_low",
        "external_high",
    }
    assert all(v["has_old"] for v in values)
    assert all(
...
def receipts():
    suffix = {
        ...
        "conformance": "native_scalar_vector_append_conformance",
    }
...
def test_sealed_native_append_receipt(receipts):
    ...
    assert (
        evidence["summary"]["executed_sites"] == 297
...
def test_exact_cli_rebuild(receipts):
    ...
    args = [
        sys.executable,
        str(ROOT / "scripts/itb_native_scalar_vector_append_conformance.py"),
        "build",
        "--executable",
        executable,
    ]
