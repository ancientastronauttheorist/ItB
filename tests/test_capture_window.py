...
@pytest.mark.skipif(os.name == "nt", reason="macOS screenshot backend test")
def test_take_fullscreen_screenshot_defaults_to_quartz_without_screencapture(
    monkeypatch,
    tmp_path,
):
    ...
    def fake_quartz(output_path: Path, *, bounds: dict | None = None) -> None:
        calls.append((output_path, bounds))
        output_path.write_bytes(b"fullscreen-quartz")
    ...
    result = window.take_fullscreen_screenshot(output_path)

    assert result == output_path
    assert output_path.read_bytes() == b"fullscreen-quartz"
    assert calls == [(output_path, None)]
