...
import pytest
from src.observatory import native_lua_class_byte_key_return_conformance as c

ROOT = Path(__file__).resolve().parents[1]
PROGRAMS = ROOT / "data/observatory/programs"
PREFIX = "windows_build_13725832_31fe35265598_"
CANONICAL = "cf3cadf46b1a06a4744ccd276ca2c7551a9d1010370cb7e6f24c52ed0b7020f4"
RAW = "505ac9eeb3f52f611b329ed22bbe7767705663628b06c88260340ca94b7c163b"
...
def test_actual_growth_resize_allocation_copy_and_free_frames(cases):
    for vector, fixture, expected in cases:
        ...
        assert events[:6] == [
            ...
            event("read", c.RECEIVER + 4, fixture["old_begin"]),
            event(
                "read", c.RECEIVER + 12, fixture["old_begin"] + 8 * vector["old_size"]
            ),
            event("write", frame - 36, fixture["old_begin"]),
            event("write", frame - 40, c.BASE + 0x2EB1DF),
        ]
        ...
        assert events[heap_call - 1] == event(
            "read", c.joined.growth.ALLOC_IAT, c.construction.IMPORT
        )
        ...
        assert (
            sum(
                e["access"] == "read" and e["address"] == c.joined.growth.FREE_IAT
                for e in events
            )
            == 1
        )
        ...
        pair = [
            event(
                "read",
                fixture["vector_begin"] + 8 * vector["argument_index"],
                read(fixture["pages"], c.ARGUMENT),
            ),
            event("write", end, read(fixture["pages"], c.ARGUMENT)),
            event(
                "read",
                fixture["vector_begin"] + 8 * vector["argument_index"] + 4,
...
def receipts():
    evidence_path = PROGRAMS / (
        PREFIX + "native_lua_class_byte_key_return_conformance.json"
    )
...
def test_seal_encoding_scope_and_coverage(receipts):
    ...
    for key, value in dict(
        ...
        max_iterations=7,
        instruction_bytes=2384,
        static_sites=924,
        executed_sites=776,
        vector_allocations=1152,
        free_requests=1152,
        grown_vector_records=4608,
    ...
    assert not any(
        a <= int(pc, 16) < b
        for pc in executed
        for a, b in ((0x2EB15F, 0x2EB179), (0x2EB1F7, 0x2EB216))
    )
    assert "failure" in " ".join(evidence["scope"]["not_claimed"])
    assert summary["key_profiles"] == {p: 384 for p in c.PROFILES}
...
def test_modified_receipt_and_source_rejected(receipts):
    ...
    changed_sources["internal_growth_return"]["schema_version"] += 1
    with pytest.raises(RuntimeError):
...
def test_exact_cli_rebuild(receipts):
    ...
    command = [
        sys.executable,
        str(ROOT / "scripts/itb_native_lua_class_byte_key_return_conformance.py"),
        "build",
        "--executable",
        executable,
    ]
...
def test_invalid_internal_vector_geometry(field, value):
    with pytest.raises(RuntimeError):
...
def test_append_reads_new_record_after_successful_free(cases):
    for vector, fixture, expected in cases:
        frame = fixture["stack"] - 4
        free_return = next(
            i
            for i, e in enumerate(expected["events"])
            if e
            == dict(
                access="write", address=frame - 128, width=4, value=c.BASE + 0x389172
            )
        )
        later = expected["events"][free_return + 1 :]
        source = fixture["vector_begin"] + 8 * vector["argument_index"]
        assert any(e["access"] == "read" and e["address"] == source for e in later)
        assert not any(
            e["access"] == "read"
            and fixture["old_begin"]
            <= e["address"]
...
def test_byte_key_layout_order_and_complete_source_protection(cases):
    profiles = set()
    saw_high = saw_empty = saw_64 = False
    for vector, fixture, expected in cases:
        profiles.add(vector["key_profile"])
        mapping = fixture["key_bytes"]
        keys = sorted(mapping)
        assert keys == sorted(set(vector["source_keys"] + vector["destination_keys"]))
        assert all(mapping[a] < mapping[b] for a, b in zip(keys, keys[1:]))
        for key, encoded in mapping.items():
            saw_high |= any(b >= 128 for b in encoded)
            saw_empty |= encoded == b""
            saw_64 |= len(encoded) == 64
        for state, addresses, base, stride in (
            (
            ...
            for i, node in enumerate(state["tree"]["nodes"]):
                pointer = read(fixture["pages"], addresses[i] + 16)
                assert pointer == base + stride * i
                payload = mapping[node["key"]] + b"\0"
                assert fixture["strings"][pointer] == payload
                assert (
                    bytes(
                        read(fixture["pages"], pointer + j, 1)
        ...
        for pointer, payload in fixture["strings"].items():
            assert (
                bytes(
                    read(expected["pages"], pointer + i, 1) for i in range(len(payload))
                )
                == payload
            )
        for page in (c.prefix.SOURCE_OBJECT, c.prefix.SOURCE_KEYS):
            if page in fixture["pages"]:
                assert expected["pages"][page] == fixture["pages"][page]
    assert profiles == set(c.PROFILES)
    assert saw_high and saw_empty and saw_64


def test_new_tree_nodes_retain_actual_source_string_pointer(cases):
    for vector, fixture, expected in cases:
...
def test_invalid_byte_key_profile(value):
    with pytest.raises(RuntimeError):
        c._fixture(dict(c.vectors()[0], key_profile=value))
