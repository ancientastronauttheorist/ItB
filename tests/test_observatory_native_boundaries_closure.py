...
def test_cleanup_receipt_closes_native_boundary_campaigns_and_owner_state():
    receipt = _load(CLEANUP_RECEIPT)
    assert receipt["kind"] == "observatory_native_boundaries_cleanup_receipt"
    ...
    assert set(campaigns) == {"selected_queue", "spawn_span"}
    pending = {
