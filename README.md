...
Status: **28 / 70 achievements earned** (latest Steam sync on 2026-05-10; refresh with `python3 game_loop.py achievements --sync` when `.env` has `STEAM_API_KEY` and `STEAM_ID`).
...
Current live-run milestone: **Hard Victory** hunt is active with Rusting Hulks on Hard + Advanced Edition. Simulator v108 includes the recent Rusting Hulks hardening stack: Aerial Bombs forest-smoke / Morgan Boost handling, Mission_Wind rows no longer draining grid as direct damage, replay snapshots preserving Boosted status, Kai Miller's state-based full-HP Boost semantics, Arachnid Psion player-phase death eggs, tile-driven conveyor movement before Vek attacks, and full-health repair-platform consumption without phantom overheal. The live loop now also treats delayed grid scalar updates as pending grid debt, waits for post-enemy grid/building reads to settle before recording favorable outcomes, preserves active weights and soft-disable masks during safety widening, and reports dirty-frontier options by exact loss magnitude. Default plan is a 2-island win attempt with an explicit post-island-2 readiness check before the Volcanic Hive.
