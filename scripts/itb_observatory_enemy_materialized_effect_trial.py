#!/usr/bin/env python3
"""Run one fixed complete selected SkillEffect materialization Observatory trial."""
...
from src.bridge.writer import bridge_observatory_enemy_materialized_effect_trial  # noqa: E402
from src.observatory.enemy_materialized_effect_hw import (  # noqa: E402
...
ACK_RE = re.compile(
    r"OK OBS_ENEMY_MATERIALIZED_EFFECT_TRIAL condition=(control|dormant|armed) "
    r"capture=([a-z][a-z0-9._-]{0,95}) pawn=(\d+) type=Firefly1 "
    r"at=([0-7]),([0-7]) consumed_spawns=(\d+) candidates=(\d+) "
    r"selected=(\d+) materialized=(\d+) queue=(\d+) complete=true\Z"
)
...
def run(args: argparse.Namespace) -> tuple[int, dict]:
    ...
    receipt = load_json_object(args.build_receipt, "enemy materialized-effect build receipt")
    ...
    try:
        ack, snapshot = bridge_observatory_enemy_materialized_effect_trial(
    ...
    trial = {
        "schema_version": 1,
        "kind": "observatory_enemy_materialized_effect_turn_trial",
        ...
        "native_counts": (
            {
                ...
                "materialized_effect_count": int(ack_match.group(9)),
                "queue_count": int(ack_match.group(10)),
            }
            if ack_match
            else None
        ),
        ...
        "snapshot": (
            {
                ...
                "materialized_effect_count": snapshot.get("summary", {}).get(
