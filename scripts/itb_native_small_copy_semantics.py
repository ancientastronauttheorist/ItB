#!/usr/bin/env python3
"""Build or verify bounded small scalar copy semantics; builds emit JSON to stdout."""
...
from src.observatory.native_small_copy_semantics import (
    SOURCE_PINS,
    SmallCopyError,
    build_semantics,
    validate_semantics,
    ...
    try:
        ...
        if args.command == "build":
            ...
            if payload != encode_semantics(evidence).encode("utf-8"):
                raise SmallCopyError("evidence is not deterministically encoded")
            result = (
