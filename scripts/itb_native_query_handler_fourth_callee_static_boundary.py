#!/usr/bin/env python3
"""Build or verify the relationship-defined query-handler fourth callee boundary."""
...
from src.observatory.native_query_handler_fourth_callee_static_boundary import (
    ANALYSIS_KIND, SCHEMA_VERSION, NativeQueryHandlerFourthCalleeStaticBoundaryError, _canonical_bytes,
    build_native_query_handler_fourth_callee_static_boundary, encode_native_query_handler_fourth_callee_static_boundary,
    validate_native_query_handler_fourth_callee_static_boundary,
    validate_native_query_handler_fourth_callee_static_boundary_structure,
)
...
def _identity(value: dict[str, Any]) -> bytes:
    if type(value.get("schema_version")) is not int or value.get("schema_version") != SCHEMA_VERSION or value.get("analysis_kind") != ANALYSIS_KIND or not isinstance(value.get("build_identity"), dict): raise NativeQueryHandlerFourthCalleeStaticBoundaryError("evidence has another schema, kind, or lacks build identity")
    prerequisite = value.get("query_handler_static_boundary")
    if not isinstance(prerequisite, dict) or type(prerequisite.get("canonical_sha256")) is not str: raise NativeQueryHandlerFourthCalleeStaticBoundaryError("evidence lacks query-handler prerequisite identity")
...
def _write_immutably_impl(output: Path, rendered: str, value: dict[str, Any]):
    ...
    if requested.parent != configured: raise NativeQueryHandlerFourthCalleeStaticBoundaryError("output must be a direct child of data observatory programs")
    ...
    if os.path.lexists(destination):
        ...
        if not _same(existing, payload, identity, expected): raise NativeQueryHandlerFourthCalleeStaticBoundaryError("refusing to overwrite differing query-handler fourth-callee evidence")
        with _locked_output(destination, resolved, (final.st_dev, final.st_ino)) as descriptor:
            ...
            if not _same(final_value, final_payload, identity, expected): raise NativeQueryHandlerFourthCalleeStaticBoundaryError("existing output changed during final content validation")
            return
    ...
    try:
        ...
        if stat.S_ISLNK(source.st_mode) or _is_reparse(source) or not stat.S_ISREG(source.st_mode): raise NativeQueryHandlerFourthCalleeStaticBoundaryError("temporary output is not a real regular file")
        ...
        with _locked_output(destination, resolved, source_identity) as descriptor:
            ...
            if not _same(final_value, final_payload, identity, expected): raise NativeQueryHandlerFourthCalleeStaticBoundaryError("created output changed during final content validation")
            return
    except Exception:
...
def main(argv=None):
    ...
    try:
        ...
        if args.command == "build":
            result = build_native_query_handler_fourth_callee_static_boundary(args.executable, *common, inventory=get("inventory")); rendered = encode_native_query_handler_fourth_callee_static_boundary(result)
            ...
            if payload != encode_native_query_handler_fourth_callee_static_boundary(evidence).encode("utf-8"): raise NativeQueryHandlerFourthCalleeStaticBoundaryError("evidence is not deterministically encoded")
            result = validate_native_query_handler_fourth_callee_static_boundary(args.executable, evidence, *common, inventory=get("inventory")) if args.command == "verify" else validate_native_query_handler_fourth_callee_static_boundary_structure(evidence, *common)
            sys.stdout.write(encode_native_query_handler_fourth_callee_static_boundary(result))
        return 0
    except (NativeQueryHandlerFourthCalleeStaticBoundaryError, NativeAssertionHelperStaticBoundaryError, NativeLuaCClosurePublicationError, NativeLuaDirectCallError, NativeLuaPropertyFactoryChainError, OSError, UnicodeError, json.JSONDecodeError) as exc: print(f"error: {exc}", file=sys.stderr); return 1
