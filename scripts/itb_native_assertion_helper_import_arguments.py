#!/usr/bin/env python3
"""Build or verify conditional import argument construction; builds emit JSON to stdout."""
...
from src.observatory.native_assertion_helper_import_arguments import (
    SOURCE_PINS,
    ArgumentError,
    build_arguments,
    validate_arguments,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "build":
            result = build_arguments(args.executable, sources)
        else:
            evidence, payload = _read_json_document(args.evidence, "evidence")
            if payload != encode_arguments(evidence).encode("utf-8"):
                raise ArgumentError("evidence is not deterministically encoded")
            result = (
                validate_structure(evidence, sources)
                if args.command == "verify-structure"
                else validate_arguments(args.executable, evidence, sources)
            )
        sys.stdout.buffer.write(encode_arguments(result).encode("utf-8"))
        return 0
