#!/usr/bin/env python3
"""Build or verify the opaque 0x00378a15 static-boundary receipt."""
...
from src.observatory.native_query_handler_first_callee_pointer_target_adjacent_callee_cluster_second_callee_static_boundary import (  # noqa: E402
    ANALYSIS_KIND,
    SCHEMA_VERSION,
    NativeQueryHandlerFirstCalleePointerTargetAdjacentCalleeClusterSecondCalleeStaticBoundaryError as BoundaryError,
    _canonical_bytes,
    build_native_query_handler_first_callee_pointer_target_adjacent_callee_cluster_second_callee_static_boundary as build,
    encode_native_query_handler_first_callee_pointer_target_adjacent_callee_cluster_second_callee_static_boundary as encode,
    validate_native_query_handler_first_callee_pointer_target_adjacent_callee_cluster_second_callee_static_boundary as validate,
    validate_native_query_handler_first_callee_pointer_target_adjacent_callee_cluster_second_callee_static_boundary_structure as validate_structure,
)
...
def _write_immutably_impl(output: Path, rendered: str, value: dict[str, Any]) -> None:
    ...
    if os.path.lexists(destination):
        ...
        if not _same_expected(existing, payload, identity, expected):
            raise BoundaryError(
                "refusing to overwrite differing adjacent-cluster second-callee evidence"
            )
        with _locked_output(
