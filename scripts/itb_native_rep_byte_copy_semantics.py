#!/usr/bin/env python3
"""Build or verify bounded forward REP byte copy semantics; builds emit JSON to stdout."""
...
from src.observatory.native_rep_byte_copy_semantics import (
    SOURCE_PINS,
    RepByteCopyError,
    build_semantics,
    validate_semantics,
    ...
    try:
        ...
        if args.command == "build":
            ...
            if payload != encode_semantics(evidence).encode("utf-8"):
                raise RepByteCopyError("evidence is not deterministically encoded")
            result = (
