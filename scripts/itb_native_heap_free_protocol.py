#!/usr/bin/env python3
"""Build or verify conditional finite heap free protocol; builds emit JSON to stdout."""
...
from src.observatory.native_heap_free_protocol import (
    SOURCE_PINS,
    HeapFreeError,
    build_semantics,
    validate_semantics,
    ...
    try:
        ...
        if args.command == "build":
            ...
            if payload != encode_semantics(evidence).encode("utf-8"):
                raise HeapFreeError("evidence is not deterministically encoded")
            result = (
