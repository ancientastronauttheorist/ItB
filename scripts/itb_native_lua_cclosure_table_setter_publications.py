#!/usr/bin/env python3
"""Build or verify exact direct-table-setter Lua C-closure publications."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for command, help_text in (
        ("build", "build exact direct-table-setter publication evidence"),
        ("verify", "verify direct-table-setter publication evidence"),
    ):
...
def _replacement_identity(value: dict[str, Any], label: str) -> bytes:
    if value.get("schema_version") != SCHEMA_VERSION or value.get("analysis_kind") != ANALYSIS_KIND:
        raise NativeLuaCClosureTableSetterPublicationError(
            f"{label} has another schema version or analysis kind"
        )
    ...
    setfield = value.get("setfield_publication_census")
    if not isinstance(identity, dict) or not all(
        isinstance(item, dict) for item in (atlas, direct, callbacks, setfield)
    ):
        raise NativeLuaCClosureTableSetterPublicationError(
            f"{label} lacks prerequisite identity"
        )
    keys = (
        atlas.get("canonical_sha256"),
        direct.get("canonical_sha256"),
        callbacks.get("canonical_sha256"),
        setfield.get("canonical_sha256"),
    )
    ...
    return _canonical_bytes(
        {
            ...
            "atlas_canonical_sha256": keys[0],
            "direct_call_census_canonical_sha256": keys[1],
            "callback_census_canonical_sha256": keys[2],
            "setfield_publication_census_canonical_sha256": keys[3],
        }
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        setfield = _read_json_object(
            args.setfield_publications, "setfield publications"
        )
        if args.command == "build":
            result = build_native_lua_cclosure_table_setter_publication_census(
                args.executable,
                direct,
                callbacks,
                setfield,
            ...
            rendered = encode_native_lua_cclosure_table_setter_publication_census(
                result
            )
            ...
            if payload != encode_native_lua_cclosure_table_setter_publication_census(
                evidence
            ).encode("utf-8"):
                raise NativeLuaCClosureTableSetterPublicationError(
                    "evidence is not deterministically encoded"
                )
            result = validate_native_lua_cclosure_table_setter_publication_census(
                args.executable,
                evidence,
                direct,
                callbacks,
            ...
            sys.stdout.write(
                encode_native_lua_cclosure_table_setter_publication_census(result)
            )
        return 0
    except (
        NativeLuaCClosureTableSetterPublicationError,
