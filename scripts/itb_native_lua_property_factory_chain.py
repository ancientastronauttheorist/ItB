#!/usr/bin/env python3
"""Build or verify the exact native Lua ``property`` factory chain."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for command, help_text in (
        ("build", "build exact property-factory evidence"),
        ("verify", "verify committed property-factory evidence"),
    ):
...
def _write_evidence_immutably(
    ...
    if destination.exists() or destination.is_symlink():
        ...
        try:
            existing, payload = _read_json_document(
            ...
            raise NativeLuaPropertyFactoryChainError(
                "refusing to replace unrelated property-factory evidence"
            ) from exc
        ...
        if (
            ...
            raise NativeLuaPropertyFactoryChainError(
                "refusing to overwrite differing property-factory evidence"
            )
        ...
        if (
            ...
            raise NativeLuaPropertyFactoryChainError(
                "existing property-factory evidence changed during comparison"
            )
        _recheck_output_root(configured, resolved, before)
        return
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        table_keys = _read_json_object(
            args.table_key_provenance, "table-key provenance"
        )
        terminal = _read_json_object(
            args.terminal_dispositions, "terminal dispositions"
        )
        common = (
            args.executable,
            direct,
        ...
        if args.command == "build":
            result = build_native_lua_property_factory_chain(
                *common, inventory=inventory
            )
            rendered = encode_native_lua_property_factory_chain(result)
            ...
            if payload != encode_native_lua_property_factory_chain(
                evidence
            ).encode("utf-8"):
                raise NativeLuaPropertyFactoryChainError(
                    "evidence is not deterministically encoded"
                )
            result = validate_native_lua_property_factory_chain(
                args.executable,
                evidence,
                direct,
                callbacks,
            ...
            sys.stdout.write(encode_native_lua_property_factory_chain(result))
        return 0
    except (
        NativeLuaPropertyFactoryChainError,
        NativeLuaDirectCallError,
