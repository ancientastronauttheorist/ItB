#!/usr/bin/env python3
"""Build or verify bounded feature-zero scalar and REP copy semantics; builds emit JSON to stdout."""
...
from src.observatory.native_scalar_copy_semantics import (
    SOURCE_PINS,
    ScalarCopyError,
    build_semantics,
    validate_semantics,
    ...
    try:
        ...
        if args.command == "build":
            ...
            if payload != encode_semantics(evidence).encode("utf-8"):
                raise ScalarCopyError("evidence is not deterministically encoded")
            result = (
