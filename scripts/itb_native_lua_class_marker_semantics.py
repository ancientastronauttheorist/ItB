#!/usr/bin/env python3
"""Build or verify conditional Lua class marker semantics; builds emit JSON to stdout."""
...
from src.observatory.native_lua_class_marker_semantics import (
    SOURCE_PINS,
    MarkerError,
    build_semantics,
    validate_semantics,
    ...
    try:
        ...
        if args.command == "build":
            ...
            if payload != encode_semantics(evidence).encode("utf-8"):
                raise MarkerError("evidence is not deterministically encoded")
            result = (
