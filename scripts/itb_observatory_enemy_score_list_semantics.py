#!/usr/bin/env python3
"""Build or verify exact shipped enemy ScoreList projection semantics."""
...
from src.observatory.enemy_score_list_semantics import (  # noqa: E402
...
def _reject_constant(value: str) -> None:
    raise EnemyScoreListSemanticsError(f"invalid JSON constant: {value}")
...
def _reject_duplicates(pairs: list[tuple[str, object]]) -> dict:
    ...
    for key, value in pairs:
        if key in result:
            raise EnemyScoreListSemanticsError(f"duplicate JSON key: {key}")
        result[key] = value
    return result
...
def _read_json(path: Path, label: str) -> dict:
    ...
    if path.stat().st_size > 16 * 1024 * 1024:
        raise EnemyScoreListSemanticsError(f"{label} exceeds size limit")
    value = json.loads(
    ...
    if not isinstance(value, dict):
        raise EnemyScoreListSemanticsError(f"{label} must contain an object")
    return value
...
def _parser() -> argparse.ArgumentParser:
    ...
    _common(verify)
    verify.add_argument("--semantics-map", required=True, type=Path)
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "build":
            value = build_enemy_score_list_semantics(
                args.content_root,
                inventory,
            )
            ...
            result = validate_enemy_score_list_semantics(
                args.content_root,
                inventory,
                _read_json(args.semantics_map, "enemy score-list semantics map"),
            )
            sys.stdout.write(encode_enemy_score_list_semantics(result))
        return 0
    except (
        EnemyScoreListSemanticsError,
        OSError,
