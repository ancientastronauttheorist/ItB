#!/usr/bin/env python3
"""Build or verify the exact-build native Mission_Piston scheduler map."""
...
from src.observatory.piston_scheduler_boundary import (  # noqa: E402
    PistonSchedulerBoundaryError,
    build_piston_scheduler_boundary_map,
    encode_piston_scheduler_boundary_map,
    validate_piston_scheduler_boundary_map,
)


def _reject_constant(value: str) -> None:
    raise PistonSchedulerBoundaryError(f"invalid JSON constant: {value}")
...
def _reject_duplicates(pairs: list[tuple[str, object]]) -> dict:
    ...
    for key, value in pairs:
        if key in result:
            raise PistonSchedulerBoundaryError(f"duplicate JSON key: {key}")
        result[key] = value
    return result
...
def _read_json(path: Path) -> dict:
    if path.is_symlink() or not path.is_file():
        raise PistonSchedulerBoundaryError(
            "Piston scheduler map is not a regular non-symlink file"
        )
    if path.stat().st_size > 16 * 1024 * 1024:
        raise PistonSchedulerBoundaryError("Piston scheduler map exceeds size limit")
    value = json.loads(
    ...
    if not isinstance(value, dict):
        raise PistonSchedulerBoundaryError(
            "Piston scheduler map must contain an object"
        )
    return value
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "build":
            value = build_piston_scheduler_boundary_map(
                args.executable,
                args.content_root,
            )
            encoded = encode_piston_scheduler_boundary_map(value)
            ...
            sys.stdout.write(encode_piston_scheduler_boundary_map(result))
        return 0
    except (
        PistonSchedulerBoundaryError,
        OSError,
