#!/usr/bin/env python3
"""Build or verify bounded tree lower bound semantics; builds emit JSON to stdout."""
...
from src.observatory.native_tree_lower_bound_semantics import (
    SOURCE_PINS,
    LowerBoundError,
    build_semantics,
    validate_semantics,
    ...
    try:
        ...
        if args.command == "build":
            ...
            if payload != encode_semantics(evidence).encode("utf-8"):
                raise LowerBoundError("evidence is not deterministically encoded")
            result = (
