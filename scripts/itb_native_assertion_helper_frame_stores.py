#!/usr/bin/env python3
"""Build or verify the assertion frame stores."""
...
from src.observatory.native_assertion_helper_frame_stores import (
    ANALYSIS_KIND,
    SCHEMA_VERSION,
    SEALED_SHA256,
    SOURCE_PINS,
    ...
    _canonical_bytes,
    _canonical_sha256,
    build_stores as build,
    encode_stores as encode,
    validate_stores as validate,
    validate_structure,
)
...
def _identity(value: dict[str, Any]) -> bytes:
    if (
        ...
        raise Error("publication requires the exact sealed stores identity")
