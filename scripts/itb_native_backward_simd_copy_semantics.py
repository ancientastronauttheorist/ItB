#!/usr/bin/env python3
"""Build or verify bounded short backward SIMD copy semantics; builds emit JSON to stdout."""
...
from src.observatory.native_backward_simd_copy_semantics import (
    SOURCE_PINS,
    BackwardSimdCopyError,
    build_semantics,
    validate_semantics,
    validate_structure,
    encode_semantics,
    ...
    try:
        ...
        if args.command == "build":
            ...
            if payload != encode_semantics(evidence).encode("utf-8"):
                raise BackwardSimdCopyError("evidence is not deterministically encoded")
            result = (
