#!/usr/bin/env python3
"""Build or verify the assertion second-child direct-callee frontier."""
...
from src.observatory.native_assertion_helper_second_child_callee_frontier import (
    ...
    SEALED_SHA256,
    SOURCE_PINS,
    FrontierError as Error,
    _canonical_bytes,
    _canonical_sha256,
    build_frontier as build,
    encode_frontier as encode,
    validate_frontier as validate,
    validate_structure,
)
...
def _identity(value: dict[str, Any]) -> bytes:
    if (
        ...
        raise Error("publication requires the exact sealed frontier identity")
