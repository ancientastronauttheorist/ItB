#!/usr/bin/env python3
"""Build or verify the assertion-helper first-callee static receipt."""
...
from src.observatory.native_assertion_helper_first_callee_static_boundary import (  # noqa: E402
    ANALYSIS_KIND,
    SCHEMA_VERSION,
    NativeAssertionHelperFirstCalleeStaticBoundaryError,
    _canonical_bytes,
    build_native_assertion_helper_first_callee_static_boundary,
    encode_native_assertion_helper_first_callee_static_boundary,
    validate_native_assertion_helper_first_callee_static_boundary,
...
def _identity(value: dict[str, Any]) -> bytes:
    if (
        ...
        raise NativeAssertionHelperFirstCalleeStaticBoundaryError(
    ...
    if (
        ...
        raise NativeAssertionHelperFirstCalleeStaticBoundaryError(
            "evidence lacks assertion-helper predecessor identity"
        )
    return _canonical_bytes(
        {
            ...
            "assertion_helper_predecessor_canonical_sha256": predecessor[
                "canonical_sha256"
            ],
        }
    )
...
def _write_immutably_impl(output: Path, rendered: str, value: dict[str, Any]) -> None:
    ...
    if os.path.lexists(destination):
        ...
        if not _same_expected(existing, payload, identity, expected):
            raise NativeAssertionHelperFirstCalleeStaticBoundaryError(
                "refusing to overwrite differing assertion-helper first-callee evidence"
            )
        with _locked_output(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "build":
            result = build_native_assertion_helper_first_callee_static_boundary(
                args.executable, *common, inventory=get("inventory")
            )
            rendered = encode_native_assertion_helper_first_callee_static_boundary(
                result
            )
            ...
            if payload != encode_native_assertion_helper_first_callee_static_boundary(
                evidence
            ).encode("utf-8"):
                raise NativeAssertionHelperFirstCalleeStaticBoundaryError(
            ...
            result = (
                ...
                else validate_native_assertion_helper_first_callee_static_boundary_structure(
                    evidence, *common
                )
            )
            sys.stdout.write(
                encode_native_assertion_helper_first_callee_static_boundary(result)
            )
        return 0
    except (
        NativeAssertionHelperFirstCalleeStaticBoundaryError,
