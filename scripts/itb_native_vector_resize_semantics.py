#!/usr/bin/env python3
"""Build or verify bounded vector resize owner semantics; builds emit JSON to stdout."""
...
from src.observatory.native_vector_resize_semantics import (
    SOURCE_PINS,
    ResizeError,
    build_semantics,
    validate_semantics,
    ...
    try:
        ...
        if args.command == "build":
            ...
            if payload != encode_semantics(evidence).encode("utf-8"):
                raise ResizeError("evidence is not deterministically encoded")
            result = (
