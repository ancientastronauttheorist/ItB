#!/usr/bin/env python3
"""Build or verify the assertion import handoff."""
...
from src.observatory.native_assertion_helper_import_handoff import (
    ANALYSIS_KIND,
    SCHEMA_VERSION,
    SEALED_SHA256,
    SOURCE_PINS,
    HandoffError as Error,
    _canonical_bytes,
    _canonical_sha256,
    build_handoff as build,
    encode_handoff as encode,
    validate_handoff as validate,
    validate_structure,
)
...
def _identity(value: dict[str, Any]) -> bytes:
    if (
        ...
        raise Error("publication requires the exact sealed handoff identity")
