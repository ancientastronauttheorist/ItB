#!/usr/bin/env python3
"""Build or verify the opaque 0x00378a40 static-boundary receipt."""
...
from src.observatory.native_query_handler_first_callee_pointer_target_adjacent_callee_cluster_fourth_callee_static_boundary import (  # noqa: E402
    ANALYSIS_KIND,
    SCHEMA_VERSION,
    NativeQueryHandlerFirstCalleePointerTargetAdjacentCalleeClusterFourthCalleeStaticBoundaryError as BoundaryError,
    _canonical_bytes,
    build_native_query_handler_first_callee_pointer_target_adjacent_callee_cluster_fourth_callee_static_boundary as build,
    encode_native_query_handler_first_callee_pointer_target_adjacent_callee_cluster_fourth_callee_static_boundary as encode,
    validate_native_query_handler_first_callee_pointer_target_adjacent_callee_cluster_fourth_callee_static_boundary as validate,
    validate_native_query_handler_first_callee_pointer_target_adjacent_callee_cluster_fourth_callee_static_boundary_structure as validate_structure,
)
...
def _identity(value: dict[str, Any]) -> bytes:
    ...
    parent = value.get("predecessor_static_boundary")
    if not isinstance(parent, dict) or type(parent.get("canonical_sha256")) is not str:
        raise BoundaryError("evidence lacks cluster-parent identity")
    second = value.get("second_callee_static_boundary")
    if not isinstance(second, dict) or type(second.get("canonical_sha256")) is not str:
        raise BoundaryError("evidence lacks second-callee identity")
    return _canonical_bytes(
        {
            "schema_version": value["schema_version"],
            "analysis_kind": value["analysis_kind"],
            "build_identity": value["build_identity"],
            "adjacent_callee_cluster_parent_canonical_sha256": parent[
                "canonical_sha256"
            ],
...
def _write_immutably_impl(output: Path, rendered: str, value: dict[str, Any]) -> None:
    ...
    if os.path.lexists(destination):
        ...
        if not _same_expected(existing, payload, identity, expected):
            raise BoundaryError(
                "refusing to overwrite differing adjacent-cluster fourth-callee evidence"
            )
        with _locked_output(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        common = (parent, second, direct, facts)
