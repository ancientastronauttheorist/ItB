#!/usr/bin/env python3
"""Build or verify the residual direct-target-set callee receipt."""
...
from src.observatory.native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_static_boundary import (  # noqa: E402
    ANALYSIS_KIND,
    SCHEMA_VERSION,
    NativeQueryHandlerFirstCalleePointerTargetResidualDirectTargetSetCalleeStaticBoundaryError,
    build_native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_static_boundary,
    encode_native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_static_boundary,
    validate_native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_static_boundary,
...
def _identity(value: dict[str, object]) -> bytes:
    ...
    residual = value.get("residual_direct_target_set_static_boundary")
    if (
    ...
    return _canonical_bytes(
        {
            ...
            "residual_direct_target_set_canonical_sha256": residual[
                "canonical_sha256"
            ],
        }
    )
...
def _write_immutably_impl(
    ...
    if os.path.lexists(destination):
        ...
        if not _same_output(existing, payload, identity, expected):
            raise NativeQueryHandlerFirstCalleePointerTargetResidualDirectTargetSetCalleeStaticBoundaryError(
                "refusing to overwrite differing residual target-set callee evidence"
            )
        with _locked_output(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "build":
            result = build_native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_static_boundary(
                args.executable,
                *common,
                inventory=get("inventory"),
            )
            rendered = encode_native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_static_boundary(
                result
            )
            ...
            if args.command == "verify":
                result = validate_native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_static_boundary(
                    args.executable,
                    evidence,
                    *common,
                    inventory=get("inventory"),
                ...
                result = validate_native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_static_boundary_structure(
                    evidence, *common
                )
            sys.stdout.write(
                encode_native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_static_boundary(
