#!/usr/bin/env python3
"""Build or verify the exact-build native corpse-classification map."""
...
from src.observatory.corpse_classification_boundary import (  # noqa: E402
    CorpseClassificationBoundaryError,
    build_corpse_classification_boundary_map,
    encode_corpse_classification_boundary_map,
    validate_corpse_classification_boundary_map,
)


def _reject_constant(value: str) -> None:
    raise CorpseClassificationBoundaryError(f"invalid JSON constant: {value}")
...
def _reject_duplicates(pairs: list[tuple[str, object]]) -> dict:
    ...
    for key, value in pairs:
        if key in result:
            raise CorpseClassificationBoundaryError(f"duplicate JSON key: {key}")
        result[key] = value
    return result
...
def _read_json(path: Path) -> dict:
    if path.is_symlink() or not path.is_file():
        raise CorpseClassificationBoundaryError(
            "corpse-classification map is not a regular non-symlink file"
        )
    if path.stat().st_size > 16 * 1024 * 1024:
        raise CorpseClassificationBoundaryError(
            "corpse-classification map exceeds size limit"
        )
    value = json.loads(
    ...
    if not isinstance(value, dict):
        raise CorpseClassificationBoundaryError(
            "corpse-classification map must contain an object"
        )
    return value
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "build":
            value = build_corpse_classification_boundary_map(
                args.executable,
                args.content_root,
            )
            encoded = encode_corpse_classification_boundary_map(value)
            ...
            sys.stdout.write(encode_corpse_classification_boundary_map(result))
        return 0
    except (
        CorpseClassificationBoundaryError,
        OSError,
