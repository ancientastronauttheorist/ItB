#!/usr/bin/env python3
"""Build or verify the deferred direct-callee pair pointer-callee receipt."""
...
from src.observatory.native_query_handler_first_callee_pointer_target_multirange_direct_callee_pair_static_boundary import (  # noqa: E402
    ANALYSIS_KIND,
    SCHEMA_VERSION,
    NativeQueryHandlerFirstCalleePointerTargetMultirangeDirectCalleePairStaticBoundaryError,
    build_native_query_handler_first_callee_pointer_target_multirange_direct_callee_pair_static_boundary,
    encode_native_query_handler_first_callee_pointer_target_multirange_direct_callee_pair_static_boundary,
    validate_native_query_handler_first_callee_pointer_target_multirange_direct_callee_pair_static_boundary,
...
def _identity(value: dict[str, object]) -> bytes:
    ...
    multirange = value.get("multirange_static_boundary")
    if (
        not isinstance(multirange, dict)
        or type(multirange.get("canonical_sha256")) is not str
    ):
        raise NativeQueryHandlerFirstCalleePointerTargetMultirangeDirectCalleePairStaticBoundaryError(
    ...
    return _canonical_bytes(
        {
            ...
            "multirange_canonical_sha256": multirange["canonical_sha256"],
        }
    )
...
def _write_immutably_impl(
    ...
    if os.path.lexists(destination):
        ...
        if not _same_output(existing, payload, identity, expected):
            raise NativeQueryHandlerFirstCalleePointerTargetMultirangeDirectCalleePairStaticBoundaryError(
                "refusing to overwrite differing direct-callee pair evidence"
            )
        with _locked_output(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        multirange, direct_calls, program_facts = (
            get(name)
            for name in (
                "multirange-static-boundary",
                "direct-calls",
        ...
        common = multirange, direct_calls, program_facts
        if args.command == "build":
            result = build_native_query_handler_first_callee_pointer_target_multirange_direct_callee_pair_static_boundary(
                args.executable,
                *common,
                inventory=get("inventory"),
            )
            rendered = encode_native_query_handler_first_callee_pointer_target_multirange_direct_callee_pair_static_boundary(
                result
            )
            ...
            if args.command == "verify":
                result = validate_native_query_handler_first_callee_pointer_target_multirange_direct_callee_pair_static_boundary(
                    args.executable,
                    evidence,
                    *common,
                    inventory=get("inventory"),
                ...
                result = validate_native_query_handler_first_callee_pointer_target_multirange_direct_callee_pair_static_boundary_structure(
                    evidence, *common
                )
            sys.stdout.write(
                encode_native_query_handler_first_callee_pointer_target_multirange_direct_callee_pair_static_boundary(
