#!/usr/bin/env python3
"""Build and attest the dormant x86 enemy-tournament HW observer."""
...
EXPORT_NAME = "luaopen_itb_observatory_enemy_tournament_hw_observer"
...
class ObserverBuildError(RuntimeError):
    """Raised when a tournament observer build cannot be trusted."""
...
def _hardware_breakpoint_plan(
    ...
    return {
        "schema_version": 1,
        "kind": "observatory_enemy_tournament_hardware_breakpoint_plan",
...
#define OBS_TOURNAMENT_SELECTOR_RVA 0x{EXPECTED_SELECTOR_RVA:08x}u
#define OBS_TOURNAMENT_SELECTOR_RVA_TEXT \"0x{EXPECTED_SELECTOR_RVA:08x}\"
#define OBS_TOURNAMENT_SELECTOR_PREBYTE_SIZE {len(EXPECTED_SELECTOR_PREBYTES)}u
#define OBS_TOURNAMENT_SELECTOR_PREBYTES_SHA256 \"{_sha256(EXPECTED_SELECTOR_PREBYTES)}\"
...
{_c_bytes('OBS_TOURNAMENT_SELECTOR_PREBYTES', EXPECTED_SELECTOR_PREBYTES)}
...
def _attest_source(source: bytes) -> dict[str, Any]:
    ...
    start = value.find("/* TOURNAMENT_HOT_PATH_BEGIN")
    end = value.find("/* TOURNAMENT_HOT_PATH_END */")
    if start < 0 or end <= start:
        raise ObserverBuildError("tournament hot-path markers are missing")
    hot = value[start:end]
    banned_hot = (
        "HeapAlloc",
        "VirtualAlloc",
    ...
    if any(item not in hot for item in required):
        raise ObserverBuildError("observer tournament VEH source contract differs")
    if (
        value.count("tournament_relocated_region_equal(") != 3
    ...
    return {
        ...
        "fixed_candidate_ring_present": "g_tournament_candidates[OBS_RECORD_CAP]"
        in value,
    }
...
def _compile_once(environment: dict[str, str], include_data: bytes) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="itb_observatory_enemy_tournament_hw_") as raw:
        temporary = Path(raw)
        (temporary / "observatory_selected_queue_hw_build.inc").write_bytes(
            include_data
        )
        dll = temporary / "itb_observatory_enemy_tournament_hw_observer.dll"
        obj = temporary / "observatory_enemy_tournament_hw_observer.obj"
...
def build_observer(args: argparse.Namespace) -> int:
    ...
    plan_path = output_root / (
        f"windows_build_{EXPECTED_BUILD_ID}_enemy_tournament_hw_plan_{plan_sha}.json"
    )
    receipt = {
        "schema_version": 1,
        "kind": "observatory_enemy_tournament_hw_observer_build",
        "observer_version": OBSERVER_VERSION,
