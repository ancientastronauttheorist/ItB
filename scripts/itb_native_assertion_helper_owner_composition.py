#!/usr/bin/env python3
"""Build or verify conditional whole-owner composition; builds emit JSON to stdout."""
...
from src.observatory.native_assertion_helper_owner_composition import (
    SOURCE_PINS,
    OwnerError,
    build_owner,
    validate_owner,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "build":
            result = build_owner(args.executable, sources)
        else:
            evidence, payload = _read_json_document(args.evidence, "evidence")
            if payload != encode_owner(evidence).encode("utf-8"):
                raise OwnerError("evidence is not deterministically encoded")
            result = (
                validate_structure(evidence, sources)
                if args.command == "verify-structure"
                else validate_owner(args.executable, evidence, sources)
            )
        sys.stdout.buffer.write(encode_owner(result).encode("utf-8"))
        return 0
