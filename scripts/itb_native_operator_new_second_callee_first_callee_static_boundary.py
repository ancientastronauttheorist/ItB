#!/usr/bin/env python3
"""Build or verify the opaque operator-new second-callee first-callee receipt."""
...
from src.observatory.native_operator_new_second_callee_first_callee_static_boundary import (  # noqa: E402
    ANALYSIS_KIND, SCHEMA_VERSION, NativeOperatorNewSecondCalleeFirstCalleeStaticBoundaryError,
    _canonical_bytes, build_native_operator_new_second_callee_first_callee_static_boundary,
    encode_native_operator_new_second_callee_first_callee_static_boundary,
    validate_native_operator_new_second_callee_first_callee_static_boundary,
    validate_native_operator_new_second_callee_first_callee_static_boundary_structure,
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for command in ("build", "verify", "verify-structure"):
        ...
        for name in ("operator-new-second-callee-static-boundary", "direct-calls", "program-facts"):
...
def _identity(value: dict[str, Any]) -> bytes:
    if type(value.get("schema_version")) is not int or value.get("schema_version") != SCHEMA_VERSION or value.get("analysis_kind") != ANALYSIS_KIND or not isinstance(value.get("build_identity"), dict):
        raise NativeOperatorNewSecondCalleeFirstCalleeStaticBoundaryError("evidence schema, kind, or build identity differs")
    ...
    if not isinstance(predecessor, dict) or type(predecessor.get("canonical_sha256")) is not str:
        raise NativeOperatorNewSecondCalleeFirstCalleeStaticBoundaryError("evidence lacks second-callee predecessor identity")
    return _canonical_bytes({"schema_version": value["schema_version"], "analysis_kind": value["analysis_kind"],
                             "build_identity": value["build_identity"], "operator_new_second_callee_predecessor_canonical_sha256": predecessor["canonical_sha256"]})
...
def _regular_child(path: Path, root: Path, identity: tuple[int, int] | None = None) -> os.stat_result:
    ...
    if stat.S_ISLNK(value.st_mode) or _is_reparse(value) or not stat.S_ISREG(value.st_mode) or path.resolve(strict=True).parent != root or (identity is not None and (value.st_dev, value.st_ino) != identity):
        raise NativeOperatorNewSecondCalleeFirstCalleeStaticBoundaryError("output failed regular-file identity check")
    return value
...
def _write_immutably_impl(output: Path, rendered: str, value: dict[str, Any]) -> None:
    ...
    if requested.parent != configured:
        raise NativeOperatorNewSecondCalleeFirstCalleeStaticBoundaryError("output must be a direct child of data observatory programs")
    ...
    if os.path.lexists(destination):
        ...
        if not _same_expected(existing, payload, identity, expected):
            raise NativeOperatorNewSecondCalleeFirstCalleeStaticBoundaryError("refusing to overwrite differing operator-new second-callee first-callee evidence")
        with _locked_output(destination, resolved, (final.st_dev, final.st_ino)) as descriptor:
            ...
            if not _same_expected(locked, locked_payload, identity, expected):
                raise NativeOperatorNewSecondCalleeFirstCalleeStaticBoundaryError("existing output changed during final content validation")
        return
    ...
    try:
        ...
        if stat.S_ISLNK(source.st_mode) or _is_reparse(source) or not stat.S_ISREG(source.st_mode):
            raise NativeOperatorNewSecondCalleeFirstCalleeStaticBoundaryError("temporary output is not a real regular file")
        ...
        with _locked_output(destination, resolved, source_identity) as descriptor:
            ...
            if not _same_expected(locked, locked_payload, identity, expected):
                raise NativeOperatorNewSecondCalleeFirstCalleeStaticBoundaryError("created output changed during final content validation")
    except Exception:
        try:
            temporary.unlink()
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        predecessor, direct, facts = (get(name) for name in ("operator-new-second-callee-static-boundary", "direct-calls", "program-facts"))
        common = (predecessor, direct, facts)
        if args.command == "build":
            result = build_native_operator_new_second_callee_first_callee_static_boundary(args.executable, *common, inventory=get("inventory"))
            rendered = encode_native_operator_new_second_callee_first_callee_static_boundary(result)
            ...
            if payload != encode_native_operator_new_second_callee_first_callee_static_boundary(evidence).encode("utf-8"):
                raise NativeOperatorNewSecondCalleeFirstCalleeStaticBoundaryError("evidence is not deterministically encoded")
            result = validate_native_operator_new_second_callee_first_callee_static_boundary(args.executable, evidence, *common, inventory=get("inventory")) if args.command == "verify" else validate_native_operator_new_second_callee_first_callee_static_boundary_structure(evidence, *common)
            sys.stdout.write(encode_native_operator_new_second_callee_first_callee_static_boundary(result))
        return 0
    except (NativeOperatorNewSecondCalleeFirstCalleeStaticBoundaryError, NativeLuaCClosurePublicationError, NativeLuaDirectCallError, NativeLuaPropertyFactoryChainError, OSError, TypeError, UnicodeError, ValueError, json.JSONDecodeError) as exc:
