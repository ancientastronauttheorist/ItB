#!/usr/bin/env python3
"""Build or verify bounded tree predecessor semantics; builds emit JSON to stdout."""
...
from src.observatory.native_tree_predecessor_semantics import (
    SOURCE_PINS,
    PredecessorError,
    build_semantics,
    validate_semantics,
    ...
    try:
        ...
        if args.command == "build":
            ...
            if payload != encode_semantics(evidence).encode("utf-8"):
                raise PredecessorError("evidence is not deterministically encoded")
            result = (
