#!/usr/bin/env python3
"""Build or verify conditional failure return chain; builds emit JSON to stdout."""
...
from src.observatory.native_assertion_helper_failure_return import (
    SOURCE_PINS,
    FailureReturnError,
    build_return,
    validate_return,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "build":
            result = build_return(args.executable, sources)
        else:
            evidence, payload = _read_json_document(args.evidence, "evidence")
            if payload != encode_return(evidence).encode("utf-8"):
                raise FailureReturnError("evidence is not deterministically encoded")
            result = (
                validate_structure(evidence, sources)
                if args.command == "verify-structure"
                else validate_return(args.executable, evidence, sources)
            )
        sys.stdout.buffer.write(encode_return(result).encode("utf-8"))
        return 0
