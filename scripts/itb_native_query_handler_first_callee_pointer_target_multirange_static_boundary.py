#!/usr/bin/env python3
"""Build or verify the deferred multi-range pointer-callee receipt."""
...
from src.observatory.native_query_handler_first_callee_pointer_target_multirange_static_boundary import (  # noqa: E402
    ANALYSIS_KIND,
    SCHEMA_VERSION,
    NativeQueryHandlerFirstCalleePointerTargetMultirangeStaticBoundaryError,
    build_native_query_handler_first_callee_pointer_target_multirange_static_boundary,
    encode_native_query_handler_first_callee_pointer_target_multirange_static_boundary,
    validate_native_query_handler_first_callee_pointer_target_multirange_static_boundary,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for command in ("build", "verify", "verify-structure"):
        ...
        for name in (
            ...
            "residual-target-set-static-boundary",
            "direct-calls",
            "program-facts",
        ):
...
def _identity(value: dict[str, object]) -> bytes:
    ...
    residual = value.get("residual_target_set_static_boundary")
    if (
        not isinstance(pointer, dict)
        or type(pointer.get("canonical_sha256")) is not str
        or not isinstance(residual, dict)
        or type(residual.get("canonical_sha256")) is not str
    ):
        raise NativeQueryHandlerFirstCalleePointerTargetMultirangeStaticBoundaryError(
    ...
    return _canonical_bytes(
        {
            ...
            "residual_target_set_canonical_sha256": residual["canonical_sha256"],
        }
    )
...
def _write_immutably_impl(
    ...
    if os.path.lexists(destination):
        ...
        if not _same_output(existing, payload, identity, expected):
            raise NativeQueryHandlerFirstCalleePointerTargetMultirangeStaticBoundaryError(
                "refusing to overwrite differing multi-range evidence"
            )
        with _locked_output(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        pointer, residual, direct_calls, program_facts = (
            get(name)
            for name in (
                "pointer-target-static-boundary",
                "residual-target-set-static-boundary",
        ...
        common = pointer, residual, direct_calls, program_facts
        if args.command == "build":
            result = build_native_query_handler_first_callee_pointer_target_multirange_static_boundary(
                args.executable,
                *common,
                inventory=get("inventory"),
            )
            rendered = encode_native_query_handler_first_callee_pointer_target_multirange_static_boundary(
                result
            )
            ...
            if args.command == "verify":
                result = validate_native_query_handler_first_callee_pointer_target_multirange_static_boundary(
                    args.executable,
                    evidence,
                    *common,
                    inventory=get("inventory"),
                ...
                result = validate_native_query_handler_first_callee_pointer_target_multirange_static_boundary_structure(
                    evidence, *common
                )
            sys.stdout.write(
                encode_native_query_handler_first_callee_pointer_target_multirange_static_boundary(
