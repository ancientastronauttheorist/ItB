#!/usr/bin/env python3
"""Build or verify exact native callbacks passed to ``lua_pushcclosure``."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    build = subparsers.add_parser("build", help="build exact callback evidence")
    ...
    verify = subparsers.add_parser("verify", help="verify callback evidence")
...
def _prepare_output_root() -> tuple[Path, Path, os.stat_result]:
    ...
    if configured_root != expected_root:
        raise NativeLuaCClosureError(
            "configured callback output root is not repository-local"
        )
    try:
    ...
    if (
        ...
        raise NativeLuaCClosureError("repository root is not a real directory")
    current = repo_root
    for part in ("data", "observatory", "programs"):
        ...
        try:
            ...
            raise NativeLuaCClosureError(
                "cannot create callback output directory"
            ) from exc
        try:
            info = current.lstat()
        except OSError as exc:
            raise NativeLuaCClosureError(
                "cannot inspect callback output directory"
            ) from exc
        if (
            ...
            raise NativeLuaCClosureError(
                "callback output chain contains a link or reparse entry"
            )
    resolved = current.resolve(strict=True)
    if not resolved.is_relative_to(repo_root.resolve(strict=True)):
        raise NativeLuaCClosureError(
            "callback output directory escapes the repository"
        )
...
def _recheck_output_root(
    ...
    try:
        ...
        raise NativeLuaCClosureError(
            "callback output directory changed during writing"
        ) from exc
    if (
...
def _replacement_identity(value: dict[str, Any], label: str) -> bytes:
    if value.get("schema_version") != SCHEMA_VERSION:
        raise NativeLuaCClosureError(f"{label} has another schema version")
    if value.get("analysis_kind") != ANALYSIS_KIND:
        raise NativeLuaCClosureError(f"{label} has another analysis kind")
    identity = value.get("build_identity")
    atlas = value.get("atlas")
    direct = value.get("direct_call_census")
    if not isinstance(identity, dict):
        raise NativeLuaCClosureError(f"{label} lacks build identity")
    if not isinstance(atlas, dict) or not isinstance(direct, dict):
        raise NativeLuaCClosureError(f"{label} lacks prerequisite identity")
    atlas_sha = atlas.get("canonical_sha256")
    direct_sha = direct.get("canonical_sha256")
    if type(atlas_sha) is not str or type(direct_sha) is not str:
        raise NativeLuaCClosureError(f"{label} lacks canonical prerequisites")
...
def _write_evidence_immutably(
    ...
    if destination.exists() or destination.is_symlink():
        if destination.is_symlink() or not destination.is_file():
            raise NativeLuaCClosureError("refusing to replace non-regular output")
        try:
        ...
        except (NativeLuaCClosureError, NativeLuaDirectCallError, OSError) as exc:
            raise NativeLuaCClosureError(
                "refusing to replace an unrelated callback artifact"
            ) from exc
        if existing_identity != new_identity:
            raise NativeLuaCClosureError(
                "refusing to replace callback evidence for other prerequisites"
            )
        if _canonical_bytes(existing) != _canonical_bytes(result):
            raise NativeLuaCClosureError(
                "refusing to overwrite differing callback evidence"
            )
        expected_payload = rendered.encode("utf-8")
        if existing_payload != expected_payload:
            raise NativeLuaCClosureError(
                "existing callback evidence is not deterministically encoded"
            )
        ...
        if (
            ...
            raise NativeLuaCClosureError(
                "existing callback evidence changed during comparison"
            )
    ...
    try:
        ...
        try:
            ...
            raise NativeLuaCClosureError(
                "could not publish callback evidence without overwriting"
            ) from exc
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "build":
            result = build_native_lua_cclosure_callback_census(
                args.executable,
                direct_calls,
                program_facts,
                inventory=inventory,
            ...
            if evidence_payload != encode_native_lua_cclosure_callback_census(
                evidence
            ).encode("utf-8"):
                raise NativeLuaCClosureError(
                    "evidence is not deterministically encoded"
                )
            result = validate_native_lua_cclosure_callback_census(
                args.executable,
                evidence,
                direct_calls,
                program_facts,
            ...
            sys.stdout.write(encode_native_lua_cclosure_callback_census(result))
        return 0
    except (
        NativeLuaCClosureError,
        NativeLuaDirectCallError,
