#!/usr/bin/env python3
"""Build or verify the opaque operator-new second-callee receipt."""
...
from src.observatory.native_operator_new_second_callee_static_boundary import (  # noqa: E402
    ANALYSIS_KIND,
    SCHEMA_VERSION,
    NativeOperatorNewSecondCalleeStaticBoundaryError,
    _canonical_bytes,
    build_native_operator_new_second_callee_static_boundary,
    encode_native_operator_new_second_callee_static_boundary,
    validate_native_operator_new_second_callee_static_boundary,
    validate_native_operator_new_second_callee_static_boundary_structure,
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for command in ("build", "verify", "verify-structure"):
        ...
        for name in ("operator-new-static-boundary", "direct-calls", "program-facts"):
...
def _identity(value: dict[str, Any]) -> bytes:
    if (
        ...
        raise NativeOperatorNewSecondCalleeStaticBoundaryError(
    ...
    if not isinstance(predecessor, dict) or type(predecessor.get("canonical_sha256")) is not str:
        raise NativeOperatorNewSecondCalleeStaticBoundaryError(
            "evidence lacks operator-new predecessor identity"
        )
    return _canonical_bytes(
        {
            ...
            "operator_new_predecessor_canonical_sha256": predecessor["canonical_sha256"],
        }
    )
...
def _write_immutably_impl(output: Path, rendered: str, value: dict[str, Any]) -> None:
    """Publish one exact receipt, retaining a concurrent replacement for audit."""
    ...
    expected, identity = rendered.encode("utf-8"), _identity(value)
    if os.path.lexists(destination):
        ...
        if not _same_expected(existing, payload, identity, expected):
            raise NativeOperatorNewSecondCalleeStaticBoundaryError(
                "refusing to overwrite differing operator-new second-callee evidence"
            )
    ...
    try:
        ...
        # Never remove destination after publication: a non-atomic recheck/unlink
        # could delete a concurrent replacement rather than our own output.
        try:
            temporary.unlink()
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "build":
            result = build_native_operator_new_second_callee_static_boundary(
                args.executable, *common, inventory=get("inventory")
            )
            rendered = encode_native_operator_new_second_callee_static_boundary(result)
            ...
            if payload != encode_native_operator_new_second_callee_static_boundary(evidence).encode("utf-8"):
                raise NativeOperatorNewSecondCalleeStaticBoundaryError(
            ...
            result = (
                ...
                else validate_native_operator_new_second_callee_static_boundary_structure(
                    evidence, *common
                )
            )
            sys.stdout.write(encode_native_operator_new_second_callee_static_boundary(result))
        return 0
    except (
        NativeOperatorNewSecondCalleeStaticBoundaryError,
        NativeLuaCClosurePublicationError,
