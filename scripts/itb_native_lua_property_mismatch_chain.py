#!/usr/bin/env python3
"""Build or verify normalized native Lua ``property`` mismatch traces."""
...
from src.observatory.native_lua_property_mismatch_chain import (  # noqa: E402
    ANALYSIS_KIND,
    SCHEMA_VERSION,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for command, help_text in (
        ("build", "build exact property-mismatch evidence"),
        ("verify", "verify committed property-mismatch evidence"),
    ):
...
def _write_evidence_immutably(
    ...
    if destination.exists() or destination.is_symlink():
        ...
        try:
            ...
            raise NativeLuaPropertyMismatchChainError(
                "refusing to replace unrelated property-mismatch evidence"
            ) from exc
        ...
        if _canonical_bytes(existing) != _canonical_bytes(result) or payload != expected:
            raise NativeLuaPropertyMismatchChainError(
                "refusing to overwrite differing property-mismatch evidence"
            )
        ...
        if (
            ...
            raise NativeLuaPropertyMismatchChainError(
                "existing property-mismatch evidence changed during comparison"
            )
        _recheck_output_root(configured, resolved, before)
        return
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "build":
            result = build_native_lua_property_mismatch_chain(
                *common, inventory=inventory
            )
            rendered = encode_native_lua_property_mismatch_chain(result)
            ...
            if payload != encode_native_lua_property_mismatch_chain(evidence).encode(
                "utf-8"
            ):
                raise NativeLuaPropertyMismatchChainError(
                    "evidence is not deterministically encoded"
                )
            result = validate_native_lua_property_mismatch_chain(
                args.executable,
                evidence,
                *common[1:],
                inventory=inventory,
            )
            sys.stdout.write(encode_native_lua_property_mismatch_chain(result))
        return 0
    except (
        NativeLuaPropertyMismatchChainError,
        NativeLuaPropertyConsumerChainError,
