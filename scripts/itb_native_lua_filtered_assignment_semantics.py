#!/usr/bin/env python3
"""Build or verify conditional Lua filtered assignment semantics; builds emit JSON to stdout."""
...
from src.observatory.native_lua_filtered_assignment_semantics import (
    SOURCE_PINS,
    AssignmentError,
    build_semantics,
    validate_semantics,
    ...
    try:
        ...
        if args.command == "build":
            ...
            if payload != encode_semantics(evidence).encode("utf-8"):
                raise AssignmentError("evidence is not deterministically encoded")
            result = (
