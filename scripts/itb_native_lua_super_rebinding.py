#!/usr/bin/env python3
"""Build or verify the exact native Lua ``super`` rebinding chain."""
...
from src.observatory.native_lua_super_rebinding import (  # noqa: E402
    ANALYSIS_KIND,
    SCHEMA_VERSION,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for command, help_text in (
        ("build", "build exact super-rebinding evidence"),
        ("verify", "verify committed super-rebinding evidence"),
    ):
...
def _prepare_output_root() -> tuple[Path, Path, os.stat_result]:
    ...
    if configured != repo_root / "data" / "observatory" / "programs":
        raise NativeLuaSuperRebindingError("configured output root is not repository-local")
    try:
        repo_info = repo_root.lstat()
    except OSError as exc:
        raise NativeLuaSuperRebindingError("repository root is unreadable") from exc
    if stat.S_ISLNK(repo_info.st_mode) or _is_reparse(repo_info) or not stat.S_ISDIR(repo_info.st_mode):
        raise NativeLuaSuperRebindingError("repository root is not a real directory")
    current = repo_root
    for part in ("data", "observatory", "programs"):
        ...
        if stat.S_ISLNK(info.st_mode) or _is_reparse(info) or not stat.S_ISDIR(info.st_mode):
            raise NativeLuaSuperRebindingError("output chain contains a link or reparse entry")
    resolved = current.resolve(strict=True)
    if not resolved.is_relative_to(repo_root.resolve(strict=True)):
        raise NativeLuaSuperRebindingError("output directory escapes repository")
...
def _recheck_output_root(configured: Path, resolved: Path, expected: os.stat_result) -> None:
    try:
        ...
        raise NativeLuaSuperRebindingError("output directory changed during writing") from exc
    if (
        ...
        raise NativeLuaSuperRebindingError("output directory changed during writing")
...
def _replacement_identity(value: dict[str, Any], label: str) -> bytes:
    if (
        ...
        raise NativeLuaSuperRebindingError(f"{label} has another schema or analysis kind")
    ...
    if not isinstance(identity, dict) or not all(isinstance(item, dict) for item in prerequisites):
        raise NativeLuaSuperRebindingError(f"{label} lacks prerequisite identity")
    hashes = [item.get("canonical_sha256") for item in prerequisites]
    if not all(type(item) is str for item in hashes):
        raise NativeLuaSuperRebindingError(f"{label} lacks canonical prerequisites")
...
def _write_evidence_immutably(output: Path, rendered: str, result: dict[str, Any]) -> None:
    ...
    if destination.exists() or destination.is_symlink():
        if destination.is_symlink() or not destination.is_file():
            raise NativeLuaSuperRebindingError("refusing to replace non-regular output")
        try:
            ...
            raise NativeLuaSuperRebindingError(
                "refusing to replace unrelated super-rebinding evidence"
            ) from exc
        ...
        if _canonical_bytes(existing) != _canonical_bytes(result) or payload != expected:
            raise NativeLuaSuperRebindingError(
                "refusing to overwrite differing super-rebinding evidence"
            )
        ...
        if (
            ...
            raise NativeLuaSuperRebindingError(
                "existing super-rebinding evidence changed during comparison"
            )
        _recheck_output_root(configured, resolved, before)
        return
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "build":
            result = build_native_lua_super_rebinding_chain(
                *common, inventory=inventory
            )
            rendered = encode_native_lua_super_rebinding_chain(result)
            ...
            if payload != encode_native_lua_super_rebinding_chain(evidence).encode("utf-8"):
                raise NativeLuaSuperRebindingError(
                    "evidence is not deterministically encoded"
                )
            result = validate_native_lua_super_rebinding_chain(
                args.executable,
                evidence,
                direct,
                callbacks,
            ...
            sys.stdout.write(encode_native_lua_super_rebinding_chain(result))
        return 0
    except (
        NativeLuaSuperRebindingError,
        NativeLuaDirectCallError,
