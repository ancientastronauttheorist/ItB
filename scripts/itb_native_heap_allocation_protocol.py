#!/usr/bin/env python3
"""Build or verify conditional finite heap allocation protocol; builds emit JSON to stdout."""
...
from src.observatory.native_heap_allocation_protocol import (
    SOURCE_PINS,
    HeapProtocolError,
    build_semantics,
    validate_semantics,
    ...
    try:
        ...
        if args.command == "build":
            ...
            if payload != encode_semantics(evidence).encode("utf-8"):
                raise HeapProtocolError("evidence is not deterministically encoded")
            result = (
