#!/usr/bin/env python3
"""Build or verify the assertion caller fill composition."""
...
from src.observatory.native_assertion_helper_caller_fill import (
    ANALYSIS_KIND,
    SCHEMA_VERSION,
    SEALED_SHA256,
    SOURCE_PINS,
    CallerError as Error,
    _canonical_bytes,
    _canonical_sha256,
    build_caller as build,
    encode_caller as encode,
    validate_caller as validate,
    validate_structure,
)
...
def _identity(value: dict[str, Any]) -> bytes:
    if (
        ...
        raise Error("publication requires the exact sealed caller identity")
