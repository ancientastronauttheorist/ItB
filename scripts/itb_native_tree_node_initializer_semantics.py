#!/usr/bin/env python3
"""Build or verify bounded tree node initializer semantics; builds emit JSON to stdout."""
...
from src.observatory.native_tree_node_initializer_semantics import (
    SOURCE_PINS,
    InitializerError,
    build_semantics,
    validate_semantics,
    ...
    try:
        ...
        if args.command == "build":
            ...
            if payload != encode_semantics(evidence).encode("utf-8"):
                raise InitializerError("evidence is not deterministically encoded")
            result = (
