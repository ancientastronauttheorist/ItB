#!/usr/bin/env python3
"""Build or verify the external-target second-callee receipt."""
...
from src.observatory.native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_external_target_second_callee_static_boundary import (  # noqa: E402
    ANALYSIS_KIND,
    SCHEMA_VERSION,
    NativeQueryHandlerFirstCalleePointerTargetResidualDirectTargetSetCalleeExternalTargetSecondCalleeStaticBoundaryError,
    build_native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_external_target_second_callee_static_boundary,
    encode_native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_external_target_second_callee_static_boundary,
    validate_native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_external_target_second_callee_static_boundary,
    validate_native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_external_target_second_callee_static_boundary_structure,
)
...
def _identity(value: dict[str, object]) -> bytes:
    if (
        ...
        raise NativeQueryHandlerFirstCalleePointerTargetResidualDirectTargetSetCalleeExternalTargetSecondCalleeStaticBoundaryError(
...
def _write_immutably_impl(
    ...
    if os.path.lexists(destination):
        ...
        if not _same_output(existing, payload, identity, expected):
            raise NativeQueryHandlerFirstCalleePointerTargetResidualDirectTargetSetCalleeExternalTargetSecondCalleeStaticBoundaryError(
                "refusing to overwrite differing second-callee evidence"
            )
        with _locked_output(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        predecessor, direct_calls, program_facts = (
            get(name)
            for name in (
                "predecessor-static-boundary",
                "direct-calls",
        ...
        common = predecessor, direct_calls, program_facts
        if args.command == "build":
            result = build_native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_external_target_second_callee_static_boundary(
                args.executable,
                *common,
                inventory=get("inventory"),
            )
            rendered = encode_native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_external_target_second_callee_static_boundary(
                result
            )
            ...
            if args.command == "verify":
                result = validate_native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_external_target_second_callee_static_boundary(
                    args.executable,
                    evidence,
                    *common,
                    inventory=get("inventory"),
                ...
                result = validate_native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_external_target_second_callee_static_boundary_structure(
                    evidence, *common
                )
            sys.stdout.write(
                encode_native_query_handler_first_callee_pointer_target_residual_direct_target_set_callee_external_target_second_callee_static_boundary(
