#!/usr/bin/env python3
"""Build or verify conditional Lua vector allocation return semantics; builds emit JSON to stdout."""
...
from src.observatory.native_lua_vector_allocation_return_semantics import (
    SOURCE_PINS,
    AllocationReturnError,
    build_semantics,
    validate_semantics,
    validate_structure,
    encode_semantics,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "build":
            result = build_semantics(args.executable, sources)
        else:
            evidence, payload = _read_json_document(args.evidence, "evidence")
            if payload != encode_semantics(evidence).encode("utf-8"):
                raise AllocationReturnError("evidence is not deterministically encoded")
            result = (
                validate_structure(evidence, sources)
                if args.command == "verify-structure"
                else validate_semantics(args.executable, evidence, sources)
            )
