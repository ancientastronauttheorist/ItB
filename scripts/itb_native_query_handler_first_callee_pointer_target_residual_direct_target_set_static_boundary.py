#!/usr/bin/env python3
"""Build or verify the residual pointer-target callee-set receipt."""
...
from src.observatory.native_query_handler_first_callee_pointer_target_residual_direct_target_set_static_boundary import (  # noqa: E402
    ANALYSIS_KIND,
    SCHEMA_VERSION,
    NativeQueryHandlerFirstCalleePointerTargetResidualDirectTargetSetStaticBoundaryError,
    _canonical_bytes,
    build_native_query_handler_first_callee_pointer_target_residual_direct_target_set_static_boundary,
    encode_native_query_handler_first_callee_pointer_target_residual_direct_target_set_static_boundary,
    validate_native_query_handler_first_callee_pointer_target_residual_direct_target_set_static_boundary,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for command in ("build", "verify", "verify-structure"):
        ...
        for name in (
            "pointer-target-static-boundary",
            "adjacent-cluster-static-boundary",
            "direct-calls",
            "program-facts",
        ):
...
def _identity(value: dict[str, object]) -> bytes:
    ...
    pointer = value.get("pointer_target_static_boundary")
    cluster = value.get("adjacent_cluster_static_boundary")
    if (
        not isinstance(pointer, dict)
        or type(pointer.get("canonical_sha256")) is not str
        or not isinstance(cluster, dict)
        or type(cluster.get("canonical_sha256")) is not str
    ):
        raise NativeQueryHandlerFirstCalleePointerTargetResidualDirectTargetSetStaticBoundaryError(
    ...
    return _canonical_bytes({
        ...
        "pointer_target_canonical_sha256": pointer["canonical_sha256"],
        "adjacent_cluster_canonical_sha256": cluster["canonical_sha256"],
    })


def _same(value: dict[str, object], payload: bytes, identity: bytes, expected: bytes) -> bool:
    return (
...
def _write_immutably_impl(output: Path, rendered: str, value: dict[str, object]) -> None:
    ...
    if os.path.lexists(destination):
        ...
        if not _same(existing, payload, identity, expected):
            raise NativeQueryHandlerFirstCalleePointerTargetResidualDirectTargetSetStaticBoundaryError(
                "refusing to overwrite differing residual target-set evidence"
            )
        with _locked_output(destination, resolved, (final.st_dev, final.st_ino)) as descriptor:
            ...
            if not _same(final_value, final_payload, identity, expected):
                raise NativeQueryHandlerFirstCalleePointerTargetResidualDirectTargetSetStaticBoundaryError(
    ...
    try:
        ...
        with _locked_output(destination, resolved, source_identity) as locked:
            _recheck_output_root(configured, resolved, before)
            final_value, final_payload = _read_locked_json_document(locked, "final created output")
            if not _same(final_value, final_payload, identity, expected):
                raise NativeQueryHandlerFirstCalleePointerTargetResidualDirectTargetSetStaticBoundaryError(
...
def _write_immutably(output: Path, rendered: str, value: dict[str, object]) -> None:
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        pointer, cluster, direct, facts = (
            get(name) for name in (
                "pointer-target-static-boundary", "adjacent-cluster-static-boundary",
                "direct-calls", "program-facts",
            )
        )
        common = pointer, cluster, direct, facts
        if args.command == "build":
            result = build_native_query_handler_first_callee_pointer_target_residual_direct_target_set_static_boundary(
                args.executable, *common, inventory=get("inventory")
            )
            rendered = encode_native_query_handler_first_callee_pointer_target_residual_direct_target_set_static_boundary(result)
            ...
            rendered = encode_native_query_handler_first_callee_pointer_target_residual_direct_target_set_static_boundary(evidence).encode("utf-8")
            if payload != rendered:
                raise NativeQueryHandlerFirstCalleePointerTargetResidualDirectTargetSetStaticBoundaryError(
                    "evidence is not deterministically encoded"
                )
            if args.command == "verify":
                result = validate_native_query_handler_first_callee_pointer_target_residual_direct_target_set_static_boundary(
                    args.executable, evidence, *common, inventory=get("inventory")
                )
            else:
                result = validate_native_query_handler_first_callee_pointer_target_residual_direct_target_set_static_boundary_structure(
                    evidence, *common
                )
            sys.stdout.write(encode_native_query_handler_first_callee_pointer_target_residual_direct_target_set_static_boundary(result))
        return 0
    except (
        NativeQueryHandlerFirstCalleePointerTargetResidualDirectTargetSetStaticBoundaryError,
        NativeAssertionHelperStaticBoundaryError,
