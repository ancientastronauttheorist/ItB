#!/usr/bin/env python3
"""Build or verify conditional Lua tree successor semantics; builds emit JSON to stdout."""
...
from src.observatory.native_lua_tree_successor_semantics import (
    SOURCE_PINS,
    SuccessorError,
    build_semantics,
    validate_semantics,
    ...
    try:
        ...
        if args.command == "build":
            ...
            if payload != encode_semantics(evidence).encode("utf-8"):
                raise SuccessorError("evidence is not deterministically encoded")
            result = (
