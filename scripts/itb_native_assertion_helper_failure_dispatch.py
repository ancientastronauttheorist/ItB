#!/usr/bin/env python3
"""Build or verify conditional failure-prefix dispatch; builds emit JSON to stdout."""
...
from src.observatory.native_assertion_helper_failure_dispatch import (
    SOURCE_PINS,
    DispatchError,
    build_dispatch,
    validate_dispatch,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "build":
            result = build_dispatch(args.executable, sources)
        else:
            evidence, payload = _read_json_document(args.evidence, "evidence")
            if payload != encode_dispatch(evidence).encode("utf-8"):
                raise DispatchError("evidence is not deterministically encoded")
            result = (
                validate_structure(evidence, sources)
                if args.command == "verify-structure"
                else validate_dispatch(args.executable, evidence, sources)
            )
        sys.stdout.buffer.write(encode_dispatch(result).encode("utf-8"))
        return 0
