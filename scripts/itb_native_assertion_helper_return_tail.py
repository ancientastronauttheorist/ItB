#!/usr/bin/env python3
"""Build or verify conditional caller return tail; builds emit JSON to stdout."""
...
from src.observatory.native_assertion_helper_return_tail import (
    SOURCE_PINS,
    TailError,
    build_tail,
    validate_tail,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "build":
            result = build_tail(args.executable, sources)
        else:
            evidence, payload = _read_json_document(args.evidence, "evidence")
            if payload != encode_tail(evidence).encode("utf-8"):
                raise TailError("evidence is not deterministically encoded")
            result = (
                validate_structure(evidence, sources)
                if args.command == "verify-structure"
                else validate_tail(args.executable, evidence, sources)
            )
        sys.stdout.buffer.write(encode_tail(result).encode("utf-8"))
        return 0
