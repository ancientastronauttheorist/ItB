#!/usr/bin/env python3
"""Build or verify conditional vector deallocation composition; builds emit JSON to stdout."""
...
from src.observatory.native_vector_deallocation_composition import (
    SOURCE_PINS,
    CompositionError,
    build_composition,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "build":
            result = build_composition(args.executable, sources)
        else:
            evidence, payload = _read_json_document(args.evidence, "evidence")
            if payload != encode_composition(evidence).encode("utf-8"):
                raise CompositionError("evidence is not deterministically encoded")
            result = (
                validate_structure(evidence, sources)
                if args.command == "verify-structure"
                else validate_composition(args.executable, evidence, sources)
            )
        sys.stdout.buffer.write(encode_composition(result).encode("utf-8"))
        return 0
