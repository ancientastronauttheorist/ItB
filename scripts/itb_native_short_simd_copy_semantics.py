#!/usr/bin/env python3
"""Build or verify bounded short forward SIMD copy semantics; builds emit JSON to stdout."""
...
from src.observatory.native_short_simd_copy_semantics import (
    SOURCE_PINS,
    ShortSimdCopyError,
    build_semantics,
    validate_semantics,
    ...
    try:
        ...
        if args.command == "build":
            ...
            if payload != encode_semantics(evidence).encode("utf-8"):
                raise ShortSimdCopyError("evidence is not deterministically encoded")
            result = (
