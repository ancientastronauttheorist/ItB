#!/usr/bin/env python3
"""Run one fixed ScorePositioning x87 control, dormant, or armed trial."""
...
from src.bridge.protocol import (  # noqa: E402
    ...
    run_observatory_score_positioning_x87_trial,
)
from src.observatory.score_positioning_x87 import (  # noqa: E402
    ANALYSIS_KIND,
...
ACK_RE = re.compile(
    ...
    r"control_word=(\d{1,5}) complete=true\Z"
)
...
def run(args: argparse.Namespace) -> tuple[int, dict]:
    ...
    receipt = load_json_object(args.build_receipt, "x87 build receipt")
    ...
    try:
        ack, snapshot = run_observatory_score_positioning_x87_trial(
    ...
    analysis = None
    analysis_error = ""
    if args.condition == "armed" and snapshot is not None:
        try:
    ...
    valid = bool(
        ...
        and (
            ...
            or (
                ...
                and analysis.get("kind") == ANALYSIS_KIND
            )
        )
    )
    trial = {
        "schema_version": 1,
        "kind": "observatory_score_positioning_x87_turn_trial",
        ...
        "boundary": (
            {
                "record_count": int(ack_match.group(7)),
                "rounding_mode": ack_match.group(8),
                "control_word": int(ack_match.group(9)),
            }
            if ack_match
            else None
        ),
        ...
        "snapshot": (
            {
                ...
                "record_count": snapshot.get("summary", {}).get("record_count"),
        ...
        "analysis": (
            {
                ...
                "rounding_mode": analysis.get("observation", {}).get(
                    "rounding_mode"
                ),
            }
            if analysis is not None and analysis_output is not None
