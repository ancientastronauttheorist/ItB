#!/usr/bin/env python3
"""Package and attest the inert, content-addressed NextPawn span controller."""
...
SOURCE = ROOT / "src" / "bridge" / "observatory_spawn_span_controller.lua"
VERSION = "observatory-spawn-span-controller/1"
...
class SpawnSpanBuildError(RuntimeError):
    """Raised when the fixed controller package cannot be reproduced safely."""
...
def _stable_bytes(path: Path, label: str) -> bytes:
    ...
    if path.is_symlink() or not path.is_file():
        raise SpawnSpanBuildError(f"{label} must be a regular file: {path}")
    before = path.stat()
    ...
    if (
        ...
        raise SpawnSpanBuildError(f"{label} changed while being read")
    return payload
...
def build(
    *, spawner_source: Path, modloader: Path, output_root: Path
) -> dict[str, object]:
    ...
    try:
        ...
        raise SpawnSpanBuildError(f"Lua source is not UTF-8: {exc}") from exc
    controller_sha = _sha256(controller)
    if _sha256(spawner) != SPAWNER_SHA256:
        raise SpawnSpanBuildError("Spawner source SHA-256 differs from pinned build")
    required_fragments = (
        f'M.VERSION = "{VERSION}"',
    ...
    if any(fragment not in controller_text for fragment in required_fragments):
        raise SpawnSpanBuildError("controller source contract differs")
    if any(
        ...
        raise SpawnSpanBuildError("controller source contains an unreviewed I/O API")
    if controller_sha not in loader_text:
        raise SpawnSpanBuildError("modloader does not pin the controller SHA-256")
    if loader_text.count(controller_sha) != 1:
        raise SpawnSpanBuildError("modloader controller SHA-256 pin is ambiguous")

    output_root = Path(os.path.abspath(output_root.expanduser()))
    if output_root.is_symlink():
        raise SpawnSpanBuildError("output root must not be a symlink")
    output_root.mkdir(parents=True, exist_ok=True)
    module_name = f"itb_observatory_spawn_span_controller_{controller_sha}.lua"
    ...
    if module_path.exists() or module_path.is_symlink() or receipt_path.exists():
        raise SpawnSpanBuildError("create-only controller output already exists")
    ...
    receipt = {
        "schema_version": 1,
        "kind": "observatory_spawn_span_controller_build",
        ...
        "runtime_mutation": "exact_Spawner.NextPawn_slot_only",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 0
    except (OSError, SpawnSpanBuildError, TypeError, ValueError) as exc:
