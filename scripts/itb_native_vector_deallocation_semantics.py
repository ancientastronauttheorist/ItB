#!/usr/bin/env python3
"""Build or verify bounded vector deallocation guard semantics; builds emit JSON to stdout."""
...
from src.observatory.native_vector_deallocation_semantics import (
    SOURCE_PINS,
    DeallocationError,
    build_semantics,
    validate_semantics,
    ...
    try:
        ...
        if args.command == "build":
            ...
            if payload != encode_semantics(evidence).encode("utf-8"):
                raise DeallocationError("evidence is not deterministically encoded")
            result = (
