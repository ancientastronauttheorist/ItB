...
- the native reachability/path API bindings, path-profile constants, Board
  ...
  occupancy, and Massive-Water path edge; and
...
- **Completed offline before `GetTargetArea`:** the exact normal/debug gate,
  ...
  `Skill_Repair` resolver are pure replayable.
- **Completed offline through the target-area callback wrapper:** Board validity,
...
coordinates. The current bridge now exports ordered enemy-zone, direct
`Board:IsDangerous`, direct `Board:IsDangerousItem`, ground-blocked facts, and
