...
3. **Terrain + tile hover calibration.** (Shipped 2026-04-20.) The `terrain_tooltip` crop region was already calibrated in `data/ui_regions.json` (bounds `[980, 635, 1225, 725]`, 2026-04-17). `capture.build_terrain_hover_plan`, `vision.PROMPT_TERRAIN_TOOLTIP`, and `parse_terrain_tooltip` were also shipped. The wire was `orchestrator.begin_research` — it only picked unit entries. Extended the picker: terrain-only queue entries (`type=""`, `terrain_id=X`) now resolve via `_find_terrain_tile(board, X)` scanning the 8×8 for a matching tile, then dispatch `build_terrain_hover_plan` with one crop (`terrain_tooltip`). Unit entries still win when both are pending (FIFO order).
4. **Forum/Reddit WebFetch step in `orchestrator.py`.** (Shipped 2026-04-20.) New `src/research/community_fetch.py` module exposes three pure helpers: `build_queries(name)` returns Steam discussions + `r/IntoTheBreach` search URLs, `normalize_notes(raw)` flattens harness-supplied excerpts into `{source, url, excerpt, confidence}` records, and `classify_confidence(tooltip_ok, wiki_ok, community_count)` picks a `confirmed|likely|speculative|none` band. `submit_research` now emits `community_queries` in its result when the target has a usable name. A new `cmd_research_attach_community <research_id> <json>` command persists normalized excerpts to `data/wiki_raw/<encoded_name>.json` under `community_notes`, merging with existing wiki output. Sub-threshold notes (confidence < 0.3) are dropped before persistence. See CLAUDE.md rule 20 for the harness sequence.
5. **Behavior-novelty route.** (Shipped 2026-04-20.) `fuzzy_detector.extract_behavior_novelty(diff)` returns unit types with any alive-field flip in the diff — the smoking-gun desyncs. `cmd_auto_turn`'s per-sub-action desync handler calls a new `_enqueue_behavior_novelty(session, diff, turn)` helper right after each `fuzzy_detector.evaluate` firing, queueing each flipped unit type with `kind="behavior_novelty"`. On the next `cmd_read`, `orchestrator.has_actionable_research(session, board)` walks the queue for pending non-background entries whose target is currently on the board and trips the research gate if found. `kind="mech_weapon"` probes stay background (they'd otherwise cage every run on turn 1). The one-action-per-desync verify contract stands — enqueue is between sub-actions, gate fires on the next player turn.
