...
predicate and Board-vector compaction. A further static continuation proves
that `IsCorpse` is one common non-vtable predicate, maps its source/lifecycle/
mutation fields, identifies mutation 12 as dormant shipped `LEADER_NECRO`, and
finds exactly 16 effective shipped corpse types already covered by Rust. Any
future live path/death campaign should therefore target matched point vectors,
damage-relative lifecycle/cleanup timing, or `AddMove` step/scheduler behavior
rather than repeat these boundaries.
...
| Target-area gate boundary | Exact-build offline replay through Lua callback eligibility | Twenty-six regions, 18 instruction windows, complete caller inventories, five method bindings, and an exact Lua census close the active/smoke/Water/`iBonusShift`/usable-skill/`IsMech` gate and name resolver index 50 as separate `Skill_Repair`. The active census is 198 assignments after excluding eight block-commented matches; materialized callback PointLists remain explicit inputs |
| Target-area callback boundary | Exact-build offline replay from a materialized Lua PointList through native return | Nine functions, 15 instruction windows, complete six-site wrapper/context caller inventories, and 12 replay vectors join `Skill +0x110` to `Board:IsValid`, prove invalid-origin cache clearing, select `GetTargetArea` versus `GetSecondTargetArea`, and stably filter only negative coordinates. Concrete Lua point construction remains an input |
...
| Spawn RNG | Source-verified `Spawner:NextPawn` span/replay ledgers, exact MSVC observable-state recovery, coordinate hardware observation, same-process RNG attribution, derived selector-state replay, and a Rust non-fabrication safeguard | Three pawn replays reproduce `Firefly2`, `Scarab2`, and `Firefly2`. Three coordinate captures prove ordered-candidate modulo selection at direct caller 60; their native windows recover pre-states `0x161229bc`/`0x495e317b`/`0x2c54aa4a` and replay exact coordinates. Ordinals 1495/1475/1450 vary because the shared stream includes presentation draws, and ordinary solver input still lacks prospective selector-time state or complete upstream replay |
...
A later read-only derivation joins the selector record at each of those three
ordinals to that checkpoint's next two native results. The three-result windows
recover one observable MSVC pre-state class apiece: `0x161229bc`, `0x495e317b`,
and `0x2c54aa4a`. Advancing each state once reproduces the captured selector
draw and exact coordinate from the preserved ordered candidates; advancing
twice more reproduces the following native window. The derived receipt is
`windows_build_13725832_owner_local_modified_20260822_spawn_coordinate_state_replay_receipt.json`.
It validates and hash-binds the immutable source and cleanup receipts. This
closes exact post-hoc selector replay for the three captured calls, not
prospective state delivery from ordinary bridge input.
...
The subsequent one-family callback campaign resolves one of those upstream
inputs for the same deterministic scenario. Its dormant command and trial host
prepare the full 65-slot controller but activate only `GetTargetArea`, reseed
the fixed Firefly mission, end the player turn through the reviewed native
gameflow helper, and require a complete enemy cycle plus slot restoration
before publishing. Three fresh processes used control/exact, exact/control,
and control/exact launch orders. Every control reported zero attempts; every
exact run accepted all nine calls with no drop, serialization error, or restore
conflict, and every paired whole-game outcome retained semantic SHA-256
`957554169ca884c49e8770255ef6dc6aac5f51fafef3f64e8cad23294240c673`.

Calls 0 through 7 originate at the native tournament's eight candidate
destinations in the same order. For origin `(x,y)`, each `FireflyAtk1`
PointList is exactly
`[(x,y-1),(x+1,y),(x,y+1),(x-1,y)]`; the corresponding native record target
is coordinate index 3. Call 8 repeats the origin and list for candidate input
5, the winner in the separately sealed selector replay. This is a deterministic
cross-campaign correlation, not a simultaneous dual-observer proof of causal
event order, and it does not generalize to another board, Pawn, weapon,
cancellation, or retarget path.

The immutable evidence, correlation, and restore receipts are
`windows_build_13725832_owner_local_modified_20260824_enemy_target_area_callback_receipt.json`
and the matching `_cleanup_receipt.json`. Cleanup stopped the game, restored
the prior 315,686-byte Mod Loader byte-for-byte, removed ten active-install
experiment artifacts plus two bridge artifacts, and regenerated a post-cleanup
...
targets, the native equal-best set or caller-29 state, or callback-time Board
predicates. Rule 532's later campaign supplies the corresponding fixed raw
SkillEffects.
...
is separate. Rule 532 now supplies the fixed raw SkillEffects, but ordinary
solver input still lacks the prospective callback state, native wrapper
intermediates, and selector-entry RNG state, so
this adds conformance coverage without a Rust change or simulator-version bump.

The following one-family campaign activates only `GetSkillEffect` against the
same fixed save, seeds, Firefly1, and counterbalanced launch orders. Every
control reports zero calls; every exact process reports the same 33 calls with
zero drops, serialization errors, or restore conflicts, installs only the 38
effect slots, and restores all 65 callback slots. Every paired whole-game
outcome again has semantic SHA-256
`957554169ca884c49e8770255ef6dc6aac5f51fafef3f64e8cad23294240c673`.

Calls 0 through 31 form eight groups of four whose origin/target pairs exactly
match the 32 target-score arguments. The source-exact ancestry artifact proves
that inherited `Skill:GetTargetScore` calls `self:GetSkillEffect` once, so they
are the fixed scenario's score-side materializations. Each raw result contains
no instant primitive and one queued one-damage, `DIR_NONE` projectile primitive
with its actual impact tile. Call 32 repeats score-side call 23 apart from call
order: origin `[5,4]`, selected target `[4,4]`, and impact `[3,4]`. That is the
separately sealed native selected input 5, while the same GetSkillEffect process
settles Firefly1 at `[5,4]` with a queued attack on `[3,4]`. Event order and the
settled action are same-process; joins to native selection and the separately
observed target/score callbacks remain deterministic cross-campaign evidence.

The immutable effect receipt is
`windows_build_13725832_owner_local_modified_20260824_enemy_skill_effect_callback_receipt.json`;
...
9. **Candidate tournament (completed for one bounded Firefly scenario).** The
   ...
   records; three final matched pairs bind the corresponding 33 raw
   `GetSkillEffect` returns to those score arguments and the settled selected
   attack. Capture another payload only for a concrete mismatch, another
   enemy/weapon/cancellation/retarget shape, or when post-wrapper losing scores,
   target-tie RNG state, native effect postprocessing, or candidate-time Board
   predicates matter.
