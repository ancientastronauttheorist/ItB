...
- **Two phases**:
  ...
  - **Phase 2 (Underground)**: Protect the Renfield Bomb (4 HP) until it detonates. The repeating source cycle is Falling Rocks, Mech-position Tentacles, clustered Falling Rocks, and crossing-path Tentacles. Both effects use `DAMAGE_DEATH` against flying, Massive, Shielded, and Frozen units before assigning Road or Lava terrain. On exact Windows build 13725832 this is numeric damage 1000: Shield/Frozen clear without absorbing it, Armor subtracts one, ACID doubles the remainder, and the HP delta clamps at zero for stock health; corpse/callback/credit settlement remains a separate native boundary.
- **Between phases**: Disabled mechs restored. Dead pilots stay dead (replaced with AI). Limited-use items restored.
- **Renfield Bomb**: 4 HP, does NOT explode if destroyed. On a non-busy update with no bomb, source queues a replacement and extends `TurnLimit` by exactly 2. `AddBomb` excludes TEAM_PLAYER-occupied, Building, and current-danger tiles, repeatedly removes random candidates until it finds an interior x/y 2–5 tile, uses any surviving edge candidate only when no interior remains, and falls back to `(4,4)` when the pool is empty. Simulator v406 preserves the exact +2 boundary and the stable snapshot's reachable candidate set without inventing native callback timing, UID, or chosen coordinate.
