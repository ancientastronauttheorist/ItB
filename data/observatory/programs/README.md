...
`windows_build_13725832_31fe35265598_native_assertion_helper_failure_dispatch.json`
binds the mismatch continuation's feature-query thunk and specifies its frame
and branch through exclusive unexecuted interrupt/fallback stops. The 96
integer-model cases contain no actual imports or interrupt executions. See
[`docs/native_failure_dispatch.md`](../../../docs/native_failure_dispatch.md).
...
records 19,744 graph cases and an exact static caller chain.
`windows_build_13725832_31fe35265598_native_lua_vector_allocation_conformance.json`
records 672 exact-byte replays across zero return, small/large requests and
two failure frontiers. Nineteen of 34 static nodes execute; all CALLs are
exclusive stop boundaries. See the
[`allocation request specification`](../../../docs/native_lua_vector_allocation.md).
...
over 672 finite-tree cases. Exact executable decoding and byte-identical
producer rebuilding passed; the helper was not executed. The domain requires
consistent acyclic topology and disjoint node, slot and return-word storage.
