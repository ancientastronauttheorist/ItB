...
pub struct Board {
    ...
    /// Subset of `env_danger`: Env_NanoStorm tiles whose one-point hit also
    /// applies ACID. Kept separate so other non-lethal warning channels do not
    /// inherit status semantics merely because kill_int=0.
    pub env_danger_acid: u64,
...
impl Board {
    ...
    /// Is this non-lethal danger tile an Env_NanoStorm damage + ACID hit?
    #[inline]
    pub fn is_env_danger_acid(&self, x: u8, y: u8) -> bool {
        let bit = 1u64 << xy_to_idx(x, y);
        self.env_danger_acid & bit != 0
    }
