...
11. **No keyboard during combat.** End Turn is the only MCP click needed per turn — dispatch the scaled coord from `click_end_turn` (or `auto_turn`'s emitted plan). The raw offset is `(95, 78)` window-relative; never paste a fixed MCP pair since the window can move. In the Codex Computer Use tool, screenshots may be window-local while `click_end_turn` emits legacy global MCP coords; after `get_app_state`, click the visible End Turn button center (or convert global→window-local) and verify the screen switches to ENEMY ACTIVITY / ENEMY TURN.
...
16. **Deployment:** `game_loop.py read` prints deploy tiles with visual notation AND MCP pixel coords. Click those coords directly; the game prompts mechs sequentially. `tile_hover.py <TILE>` for single-tile lookup. If a deploy click does not change the "Deploying <next mech>" header or `read` shows no newly placed mech, stop re-clicking that stale coordinate: use the visible yellow drop-zone tile centers to finish deployment, then log/fix the calibration drift after the mission.
