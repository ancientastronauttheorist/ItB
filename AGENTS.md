...
41. **Lookahead frontier is diagnostic until promoted.** `project_plan_scenarios` returns bounded plausible next-turn enemy-intent scenarios: base `heuristic_requeue` plus a few high-value building retargets. `lookahead_frontier` in solve output previews worst projected recovery across those scenarios, but it does not select live actions yet. Treat it as evidence for dirty-plan triage and future robust-beam tuning, not as permission to bypass safety.
